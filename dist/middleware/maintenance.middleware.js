 ...
 */
export async function maintenanceModeGuard(request, reply) {
    ...
    try {
        ...
        let userRole = request.user?.role || request.user?.permissions?.role;
        if (!userRole && request.headers.authorization) {
            try {
                await request.jwtVerify();
                userRole = request.user?.role || request.user?.permissions?.role;
            }
            catch {
