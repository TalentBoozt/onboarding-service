...
export class SchedulerService {
    ...
    async triggerScan() {
        ...
        for (const orgId of activeUsers) {
            await queueService.enqueue("scan_overdue_assignments", { organizationId: orgId.toString() }, {
                organizationId: orgId.toString(),
                idempotencyKey: `scan_overdue_${new Date().toISOString().substring(0, 13)}`, // Once per hour per org
            });
            await queueService.enqueue("scan_compliance_due_alerts", { organizationId: orgId.toString() }, {
                organizationId: orgId.toString(),
                idempotencyKey: `scan_compliance_${new Date().toISOString().substring(0, 13)}`,
            });
            await queueService.enqueue("scan_overdue_tasks", { organizationId: orgId.toString() }, {
                organizationId: orgId.toString(),
                idempotencyKey: `scan_overdue_tasks_${new Date().toISOString().substring(0, 13)}_${orgId.toString()}`,
            });
            await queueService.enqueue("evaluate_onboarding_health_and_nudge", { organizationId: orgId.toString() }, {
                organizationId: orgId.toString(),
                idempotencyKey: `scan_health_nudge_${new Date().toISOString().substring(0, 13)}_${orgId.toString()}`,
            });
            await queueService.enqueue("scan_pending_milestone_reviews", { organizationId: orgId.toString() }, {
                organizationId: orgId.toString(),
                idempotencyKey: `scan_milestones_${new Date().toISOString().substring(0, 13)}_${orgId.toString()}`,
            });
            await queueService.enqueue("scan_buddy_coaching_nudges", { organizationId: orgId.toString() }, {
                organizationId: orgId.toString(),
