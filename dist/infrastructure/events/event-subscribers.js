...
export function registerEventSubscribers() {
    ...
    eventBus.subscribe("JOURNEY_ASSIGNED", async (event) => {
        ...
        if (empId) {
            ...
            await notificationService.notifyJourneyAssignment(event.organizationId, empId, journeyTitle || "Onboarding Journey", assignmentId || event.entityId, journeyId);
            ...
            try {
                ...
                if (!mgrId || !empName) {
                    ...
                    mgrId = mgrId || empUser?.employment?.managerId || empUser?.employment?.managerUserId;
    ...
    eventBus.subscribe("JOURNEY_COMPLETED", async (event) => {
        ...
        if (event.organizationId && empId) {
            try {
                await gamificationService.awardPoints(event.organizationId, empId, "journey_completed", 100, `Graduated onboarding journey: "${journeyTitle || "Onboarding Journey"}"`, `journey_${journeyId || assignmentId || event.entityId}`);
            }
            catch (gErr) {
        ...
        if (empId) {
            ...
            await notificationService.notifyJourneyCompletion(event.organizationId, empId, name || "Employee", journeyTitle || "Onboarding Journey", assignmentId || event.entityId, journeyId, mgrId);
        }
    });
    ...
    eventBus.subscribe("JOURNEY_OVERDUE", async (event) => {
        ...
        if (empId) {
            ...
            try {
                const empUser = await User.findById(empId).select("employment.managerId profile.firstName profile.lastName");
                const mgrId = empUser?.employment?.managerId || empUser?.employment?.managerUserId;
    ...
    eventBus.subscribe("TASK_COMPLETED", async (event) => {
        ...
        if (event.organizationId && performerId) {
            try {
                await gamificationService.awardPoints(event.organizationId, performerId, "task_completed", 25, `Completed onboarding task: "${title || "Operational Task"}"`, `task_${taskId || event.entityId}`);
            }
            catch (err) {
    ...
    eventBus.subscribe("TASK_REVISION_REQUESTED", async (event) => {
    ...
    eventBus.subscribe("TASK_NEEDS_REVIEW", async (event) => {
        const { title, taskId, employeeId, note } = event.payload || {};
        if (employeeId) {
            try {
                const empUser = await User.findById(employeeId).select("employment.managerId profile.firstName profile.lastName");
    ...
    eventBus.subscribe("DOCUMENT_SIGNED", async (event) => {
        ...
        if (event.organizationId && recipientId) {
            try {
                await gamificationService.awardPoints(event.organizationId, recipientId, "document_signed", 50, `Completed and signed compliance document: "${templateTitle || "Document"}"`, `doc_${assignmentId || event.entityId}`);
            }
            catch (err) {
    ...
    eventBus.subscribe("MILESTONE_COMPLETED", async (event) => {
        ...
        if (event.organizationId && recipientId) {
            try {
                await gamificationService.awardPoints(event.organizationId, recipientId, "milestone_completed", 50, `Achieved milestone: "${milestoneTitle || `Day ${targetDay || 30} Milestone`}"`, `milestone_${milestoneId || event.entityId}`);
            }
            catch (err) {
                console.warn("[EventSubscribers] Could not award gamification points for MILESTONE_COMPLETED:", err);
            }
            try {
                await workflowEngine.processEvent(event.organizationId, "milestone_completed", recipientId, event.payload);
            }
            catch (wfErr) {
    ...
    queueService.registerWorker("resume_delayed_workflow", async (job) => {
    ...
    const handleUserCreated = async (event) => {
        if (event.actorId) {
            await workflowEngine.processEvent(event.organizationId, "user_created", event.actorId, event.payload);
            await documentService.autoAssignDocumentsToNewHire(event.organizationId, event.actorId);
            await milestoneService.autoAssignMilestonesToNewHire(event.organizationId, event.actorId);
            await buddyService.autoAssignBuddyToNewHire(event.organizationId, event.actorId);
            await calendarService.autoScheduleOnboardingMeetings(event.organizationId, event.actorId);
            await roleChecklistService.autoAssignRoleChecklistsToNewHire(event.organizationId, event.actorId).catch((err) => console.warn("[EventSubscribers] Role checklist auto-assign error:", err));
        }
    };
    eventBus.subscribe("USER_CREATED", handleUserCreated);
    eventBus.subscribe("ON_USER_CREATED", handleUserCreated);
    // Workflow Engine Listener for JOURNEY_COMPLETED event
    eventBus.subscribe("JOURNEY_COMPLETED", async (event) => {
        if (event.actorId) {
            await workflowEngine.processEvent(event.organizationId, "journey_completed", event.actorId, event.payload);
        }
    });
    // Workflow Engine Listener for TASK_COMPLETED event
    eventBus.subscribe("TASK_COMPLETED", async (event) => {
        if (event.actorId) {
            await workflowEngine.processEvent(event.organizationId, "task_completed", event.actorId, event.payload);
        }
    });
    // Reactive State Machine: ONBOARDING_CASE_CREATED transitions case from created -> resolving -> provisioning -> ready -> active
    eventBus.subscribe("ONBOARDING_CASE_CREATED", async (event) => {
        ...
        try {
            ...
            if (caseRecord && caseRecord.state === "created") {
                await onboardingCaseService.transition(caseRecord._id.toString(), organizationId.toString(), "resolving", event.actorId?.toString(), "Resolving onboarding journey and operational rules");
                await onboardingCaseService.transition(caseRecord._id.toString(), organizationId.toString(), "provisioning", event.actorId?.toString(), "Provisioning checklist tasks, compliance documents, and milestones");
                await onboardingCaseService.transition(caseRecord._id.toString(), organizationId.toString(), "ready", event.actorId?.toString(), "Initial provisioning complete");
                await onboardingCaseService.transition(caseRecord._id.toString(), organizationId.toString(), "active", event.actorId?.toString(), "Employee onboarding active");
            }
        }
        catch (err) {
    ...
    const handlePreboardingHardwareTrigger = async (event) => {
        ...
        try {
            ...
            if (user && user.employment?.hireDate) {
                await itHardwareService.triggerPreboardingItSetup(organizationId, user._id, user.employment.hireDate);
            }
        }
        catch (err) {
    ...
    const handleProfileReEvaluation = async (event) => {
        ...
        try {
            ...
            for (const assignment of activeAssignments) {
                ...
                const matchesCurrentDept = audienceDepts.length === 0 ||
                ...
                if (!matchesCurrentDept) {
                    ...
                    if (completion < 20) {
                        ...
                        await workflowEngine.processEvent(organizationId, "user_created", employeeId, event.payload || {});
                        await notificationService.createNotification({
                            organizationId,
                            recipientUserId: employeeId,
                            type: "announcement",
    ...
    eventBus.subscribe("employee.profile_updated", handleProfileReEvaluation);
    // =========================================================================
    ...
    eventBus.subscribe("DOCUMENT_SIGNED", async (event) => {
        try {
            ...
            const isValidSha256 = typeof signatureHash === "string" &&
            ...
            for (const task of matchingTasks) {
                if (!isValidSha256) {
                    ...
                    const managerId = employee?.employment?.managerId || employee?.employment?.managerUserId;
                    if (managerId) {
                        await notificationService.createNotification({
                            organizationId,
    ...
    const handleQuizOrLmsCompletion = async (event) => {
        try {
            const organizationId = event.organizationId;
            const { employeeId, userId, quizId, courseId, score, scorePercent, scorePercentage, passingScore } = event.payload || {};
