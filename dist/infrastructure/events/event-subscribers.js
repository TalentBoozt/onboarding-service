...
export function registerEventSubscribers() {
    ...
    eventBus.subscribe("JOURNEY_ASSIGNED", async (event) => {
        ...
        if (event.actorId) {
            await notificationService.notifyJourneyAssignment(event.organizationId, event.actorId, journeyTitle || "Onboarding Journey", assignmentId || event.entityId, journeyId);
        }
    });
    ...
    eventBus.subscribe("JOURNEY_COMPLETED", async (event) => {
        ...
        if (event.organizationId && event.actorId) {
            try {
                await gamificationService.awardPoints(event.organizationId, event.actorId, "journey_completed", 100, `Graduated onboarding journey: "${journeyTitle || "Onboarding Journey"}"`, `journey_${journeyId || assignmentId || event.entityId}`);
            }
            catch (gErr) {
        ...
        if (event.actorId) {
            await notificationService.notifyJourneyCompletion(event.organizationId, event.actorId, employeeName || "Employee", journeyTitle || "Onboarding Journey", assignmentId || event.entityId, journeyId, managerUserId);
        }
    });
    ...
    eventBus.subscribe("TASK_COMPLETED", async (event) => {
        ...
        if (event.organizationId && recipientId) {
            try {
                await gamificationService.awardPoints(event.organizationId, recipientId, "task_completed", 25, `Completed onboarding task: "${title || "Operational Task"}"`, `task_${taskId || event.entityId}`);
            }
            catch (err) {
