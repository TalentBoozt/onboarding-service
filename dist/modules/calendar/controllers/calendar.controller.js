...
export class CalendarController {
    ...
    exportOnboardingPack = async (request, reply) => {
        const user = request.user;
        const query = request.query || {};
        const targetUserId = query.userId || user.userId;
        const icsContent = await this.calendarService.generateOnboardingSchedulePack(user.organizationId, targetUserId);
        return reply
            .header("Content-Type", "text/calendar; charset=utf-8")
            .header("Content-Disposition", 'attachment; filename="talnova-onboarding-pack.ics"')
            .send(icsContent);
    };
    scheduleMilestoneReview = async (request, reply) => {
        const user = request.user;
        const params = request.params;
        const body = request.body || {};
        const event = await this.calendarService.scheduleMilestoneReviewMeeting(user.organizationId, user.userId, {
