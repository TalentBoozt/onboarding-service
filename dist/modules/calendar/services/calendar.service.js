...
import CalendarConnection from "../models/calendar-connection.model.js";
import MeetingEvent from "../models/meeting-event.model.js";
...
export class CalendarService {
     ...
     */
    async connectProvider(orgId, userId, provider = "ical", timezone = "UTC") {
     ...
     */
    async getConnection(orgId, userId) {
     ...
     */
    async generateICalFeed(icalToken) {
        ...
        for (const event of events) {
            icsContent.push("BEGIN:VEVENT", `UID:${event.iCalUid}`, `DTSTAMP:${formatICalDate(event.createdAt)}`, `DTSTART:${formatICalDate(event.startTime)}`, `DTEND:${formatICalDate(event.endTime)}`, `SUMMARY:${event.title}`, `DESCRIPTION:${(event.description || "").replace(/\n/g, "\\n")}`, `LOCATION:${event.locationUrl || "Talnova Workspace"}`, `STATUS:${event.status.toUpperCase()}`, "END:VEVENT");
        }
     ...
     */
    async getAvailability(orgId, params) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
     ...
     */
    async createMeetingEvent(orgId, organizerUserId, data) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
     ...
     */
    async generateSingleEventICal(eventId) {
     ...
     */
    async listMeetingEvents(orgId, userId) {
     ...
     */
    async updateMeetingEvent(orgId, eventId, data) {
        const event = await MeetingEvent.findOne({
            _id: new mongoose.Types.ObjectId(eventId),
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        ...
        if (data.locationUrl !== undefined)
            event.locationUrl = data.locationUrl;
        if (data.status)
     ...
     */
    async cancelMeetingEvent(orgId, eventId) {
        const event = await MeetingEvent.findOne({
            _id: new mongoose.Types.ObjectId(eventId),
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
     ...
     */
    async autoScheduleOnboardingMeetings(orgId, newHireUserId) {
        ...
        if (managerId)
            attendees.push(managerId.toString());
     ...
     */
    async generateOnboardingSchedulePack(orgId, userId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const userObjectId = new mongoose.Types.ObjectId(userId);
        ...
        for (const ev of events) {
            icsLines.push("BEGIN:VEVENT", `UID:${ev.iCalUid}`, `DTSTAMP:${formatICalDate(ev.createdAt || new Date())}`, `DTSTART:${formatICalDate(ev.startTime)}`, `DTEND:${formatICalDate(ev.endTime)}`, `SUMMARY:${ev.title}`, `DESCRIPTION:${(ev.description || ev.notes || "Onboarding meeting").replace(/\n/g, "\\n")}`, `LOCATION:${ev.locationUrl || "Google Meet"}`, `STATUS:${ev.status.toUpperCase()}`, "END:VEVENT");
        }
        ...
        for (const m of milestones) {
            ...
            icsLines.push("BEGIN:VEVENT", `UID:milestone-${m._id}@talnova.app`, `DTSTAMP:${formatICalDate(new Date())}`, `DTSTART;VALUE=DATE:${dueStr}`, `DTEND;VALUE=DATE:${nextDayStr}`, `SUMMARY:🎯 Onboarding Milestone Day ${m.targetDay}: ${m.milestoneTitle}`, `DESCRIPTION:Check-in checkpoint for Day ${m.targetDay}. Goals: ${m.goalsProgress.map((g) => g.goalTitle).join("; ")}`, "STATUS:CONFIRMED", "END:VEVENT");
        }
     ...
     */
    async scheduleMilestoneReviewMeeting(orgId, schedulerUserId, params) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        ...
        const managerId = employee.employment?.managerId || employee.employment?.managerUserId || schedulerUserId;
