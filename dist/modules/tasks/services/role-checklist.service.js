...
import RoleChecklistTemplate from "../models/role-checklist-template.model.js";
...
export class RoleChecklistService {
     ...
     */
    async resolveResponsibleUser(orgId, targetUser, role, fallbackCreatorId) {
        const targetUserId = targetUser._id;
        if (!role || role === "employee") {
            return targetUserId;
        }
        if (role === "manager") {
            const managerId = targetUser.employment?.managerId || targetUser.employment?.managerUserId;
    ...
    async createTemplate(orgId, creatorId, data) {
        ...
        const template = new RoleChecklistTemplate({
            ...
            audience: {
                ...
                autoAssignNewHires: audienceData.autoAssignNewHires ?? data.autoAssignOnCreate ?? true,
            },
            items: itemsData,
    ...
    async updateTemplate(orgId, templateId, updaterId, data) {
        const template = await RoleChecklistTemplate.findOne({
            _id: new mongoose.Types.ObjectId(templateId.toString()),
            organizationId: new mongoose.Types.ObjectId(orgId.toString()),
            isDeleted: false,
     ...
     */
    async autoAssignRoleChecklistsToNewHire(orgId, userId, options) {
        ...
        for (const tmpl of activeTemplates) {
            ...
            if (aud.jobTitleNames && aud.jobTitleNames.length > 0) {
                const matchesTitle = aud.jobTitleNames.some((t) => t.toLowerCase().trim() === userJobTitle);
                if (!matchesTitle) {
                    continue;
                }
            }
            ...
            if (aud.employmentTypes && aud.employmentTypes.length > 0) {
                if (userEmpType && !aud.employmentTypes.includes(userEmpType)) {
                    continue;
                }
            }
            // Location check
            if (aud.locations && aud.locations.length > 0) {
                const matchesLoc = aud.locations.some((loc) => loc.toLowerCase().trim() === userLocation);
                if (!matchesLoc) {
                    continue;
                }
            }
        ...
        for (const tmpl of matchedTemplates) {
            ...
            const itemIndexToTaskId = new Map();
            for (let i = 0; i < tmpl.items.length; i++) {
                ...
                const assignedUserId = await this.resolveResponsibleUser(new mongoose.Types.ObjectId(orgId.toString()), user, item.responsibleRole, options?.creatorId ? new mongoose.Types.ObjectId(options.creatorId.toString()) : tmpl.createdBy);
                ...
                const managerId = user.employment?.managerId || user.employment?.managerUserId;
     ...
     */
    async applyTemplateToUser(orgId, templateId, userId, assignedBy, referenceDate) {
        ...
        for (let i = 0; i < template.items.length; i++) {
            ...
            const assignedUserId = await this.resolveResponsibleUser(new mongoose.Types.ObjectId(orgId.toString()), user, item.responsibleRole, new mongoose.Types.ObjectId(assignedBy.toString()));
            const newTask = new Task({
                organizationId: new mongoose.Types.ObjectId(orgId.toString()),
                assignedToUserId: assignedUserId,
                employeeId: new mongoose.Types.ObjectId(userId.toString()),
