...
export class ItHardwareService {
     ...
     */
    async triggerPreboardingItSetup(orgId, employeeId, hireDate, options) {
        ...
        try {
            ...
            if (org?.integrations?.mdmWebhookUrl) {
     ...
     */
    async dispatchMdmWebhook(orgId, taskId, customPayload) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
     ...
     */
    async handleMdmCallback(orgId, callbackData) {
        ...
        if (callbackData.serialNumber)
            task.hardwareMetadata.serialNumber = callbackData.serialNumber.trim();
        if (callbackData.assetTag)
            task.hardwareMetadata.assetTag = callbackData.assetTag.trim();
        if (callbackData.courierTrackingUrl)
            task.hardwareMetadata.courierTrackingUrl = callbackData.courierTrackingUrl.trim();
        if (callbackData.courierProvider)
            task.hardwareMetadata.courierProvider = callbackData.courierProvider.trim();
        if (callbackData.mdmExternalId)
            task.hardwareMetadata.mdmExternalId = callbackData.mdmExternalId;
        const effectiveMdmStatus = (callbackData.mdmStatus || callbackData.status);
        if (effectiveMdmStatus)
            task.hardwareMetadata.mdmStatus = effectiveMdmStatus;
     ...
     */
    async attachHardwareReceipt(orgId, taskId, receiptData, actingUserId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const taskObjectId = new mongoose.Types.ObjectId(taskId);
        const task = await Task.findOne({
            _id: taskObjectId,
     ...
     */
    async updateHardwareMetadata(orgId, taskId, metadata, actingUserId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const taskObjectId = new mongoose.Types.ObjectId(taskId);
        const task = await Task.findOne({
            _id: taskObjectId,
        ...
        if (metadata.deviceType)
            task.hardwareMetadata.deviceType = metadata.deviceType;
        if (metadata.serialNumber !== undefined)
            task.hardwareMetadata.serialNumber = metadata.serialNumber.trim();
        if (metadata.assetTag !== undefined)
            task.hardwareMetadata.assetTag = metadata.assetTag.trim();
        if (metadata.courierTrackingUrl !== undefined)
            task.hardwareMetadata.courierTrackingUrl = metadata.courierTrackingUrl.trim();
        if (metadata.courierProvider !== undefined)
            task.hardwareMetadata.courierProvider = metadata.courierProvider.trim();
        if (metadata.shipDate)
            task.hardwareMetadata.shipDate = new Date(metadata.shipDate);
        if (metadata.mdmStatus)
            task.hardwareMetadata.mdmStatus = metadata.mdmStatus;
        if (metadata.mdmExternalId)
            task.hardwareMetadata.mdmExternalId = metadata.mdmExternalId;
     ...
     */
    async confirmHardwareReceipt(orgId, taskId, userId, note) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const taskObjectId = new mongoose.Types.ObjectId(taskId);
