...
export class KnowledgeGapController {
     ...
     */
    async resolveWithQuickAnswer(req, reply) {
        const user = req.user;
        const { id } = req.params;
        ...
        const updatedGap = await this.gapService.resolveWithQuickAnswer(user.organizationId, id, answer.trim(), user.userId);
        return reply.send({
            success: true,
            data: updatedGap,
        });
     ...
     */
    async resolveWithArticle(req, reply) {
        const user = req.user;
        const { id } = req.params;
        ...
        const updatedGap = await this.gapService.resolveWithArticle(user.organizationId, id, articleId, user.userId);
        return reply.send({
            success: true,
            data: updatedGap,
        });
     ...
     */
    async dismissGap(req, reply) {
        const user = req.user;
        const { id } = req.params;
        const updatedGap = await this.gapService.dismissGap(user.organizationId, id, user.userId);
        return reply.send({
            success: true,
            data: updatedGap,
        });
