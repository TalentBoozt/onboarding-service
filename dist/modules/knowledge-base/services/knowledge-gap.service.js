...
import KnowledgeGap from "../models/knowledge-gap.model.js";
...
export class KnowledgeGapService {
     ...
     */
    async notifyAdminsOfGap(orgId, question) {
     ...
     */
    async resolveWithQuickAnswer(orgId, gapId, answerText, userId) {
     ...
     */
    async resolveWithArticle(orgId, gapId, articleId, userId) {
        const gap = await KnowledgeGap.findOne({
            _id: gapId,
            organizationId: orgId,
        });
     ...
     * Dismiss gap
     */
    async dismissGap(orgId, gapId, userId) {
        const gap = await KnowledgeGap.findOne({
            _id: gapId,
            organizationId: orgId,
        });
