...
export class KnowledgeIndexingService {
     ...
     */
    async indexArticle(article, providerConfig, secrets) {
        const orgId = article.organizationId;
        const resourceId = article._id;
        // 1. Deactivate prior chunks for this resource
        await KnowledgeChunk.updateMany({ organizationId: orgId, resourceId }, { $set: { status: "inactive" } });
        ...
        for (let i = 0; i < rawChunks.length; i++) {
            ...
            const embedding = await this.embeddingService.generateEmbedding(`${article.title} - ${chunk.heading || ""}: ${chunk.content}`, providerConfig, secrets);
            chunkDocs.push({
     ...
     */
    async deindexArticle(orgId, resourceId) {
        await KnowledgeChunk.updateMany({ organizationId: orgId, resourceId }, { $set: { status: "inactive" } });
    }
    /**
     * Create and immediately index a Quick Answer as an active knowledge chunk.
     */
    async indexQuickAnswer(orgId, resourceId, question, answer) {
        ...
        // Deactivate previous
        await KnowledgeChunk.updateMany({ organizationId: orgObjectId, resourceId: resObjectId }, { $set: { status: "inactive" } });
