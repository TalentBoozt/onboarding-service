...
import OnboardingCase from "../models/onboarding-case.model.js";
...
export class OnboardingCaseService {
    notificationService;
    assignmentService;
    constructor(notificationService, assignmentService) {
    ...
    async createCase(input) {
        ...
        try {
            ...
            if (error?.code === 11000) {
                ...
                if (duplicate)
                    return { case: duplicate, created: false };
            }
            throw error;
        }
    }
    async transition(caseId, organizationId, to, actorUserId, reason) {
        ...
        if (!transitions[record.state].includes(to)) {
            throw new AppError(409, "INVALID_STATE_TRANSITION", `Cannot transition onboarding case from ${record.state} to ${to}`);
        }
    ...
    async cancelCaseForEmployee(organizationId, employeeId, reason = "hris_termination_event", actorUserId) {
     ...
     */
    async getExceptionCases(organizationId, filter) {
        const orgObjectId = new mongoose.Types.ObjectId(organizationId.toString());
        ...
        const cases = rawCases.map((c) => {
            ...
            const lastTransition = c.transitions && c.transitions.length > 0
                ? c.transitions[c.transitions.length - 1]
                : null;
            const quarantinedAt = lastTransition?.at || c.updatedAt || c.createdAt;
            const daysQuarantined = Math.max(0, Math.floor((now - new Date(quarantinedAt).getTime()) / (1000 * 60 * 60 * 24)));
            let severity = "medium";
     ...
     */
    async resolveException(caseId, organizationId, actorUserId, resolution, reqMeta) {
        ...
        if (!resolution.reason || resolution.reason.trim().length < 10) {
            throw new AppError(400, "VALIDATION_ERROR", "A detailed resolution reason of at least 10 characters is mandatory for audit compliance.");
        }
        const orgObjectId = new mongoose.Types.ObjectId(organizationId.toString());
        ...
        if (resolution.employmentUpdates) {
            ...
            if (resolution.employmentUpdates.managerId) {
                employee.employment.managerId = new mongoose.Types.ObjectId(resolution.employmentUpdates.managerId);
            }
            await employee.save();
        }
        ...
        if (resolution.action === "override_journey") {
            if (!resolution.targetTemplateId) {
                throw new AppError(400, "VALIDATION_ERROR", "targetTemplateId is required when overriding a journey.");
            }
            ...
            await this.assignmentService.assignJourney(orgObjectId, employee._id, targetJourney._id, actorObjectId, {
                source: "manual",
                priority: "normal",
            });
            ...
            await OnboardingHealth.updateOne({ organizationId: orgObjectId, employeeId: employee._id }, {
                $set: {
                    lastActiveAt: now,
                    calculatedAt: now,
                    nudgeLevel: 0,
