        ...
        for (let i = 0; i < limit; i++) {
            ...
            try {
                ...
                if (item.aggregateType === "hris_integration") {
                    const rawData = item.payload?.data || item.payload || {};
                    ...
                    if (item.eventName === "hris.employee.created") {
                        ...
                        if (user) {
                            ...
                            await workflowEngine.processEvent(item.organizationId, "user_created", user._id, { department: user.employment?.department });
                            await documentService.autoAssignDocumentsToNewHire(item.organizationId, user._id);
                        }
                        if (item.payload?.eventId) {
                            await HRISWebhookLog.updateOne({ organizationId: item.organizationId, eventId: item.payload.eventId }, { $set: { status: "processed", processedAt: new Date() } });
                        }
                        ...
                        await hrisService.executeTerminationLifecycle(item.organizationId, targetIdentifier, "hris_termination_event");
                        if (item.payload?.eventId) {
                            await HRISWebhookLog.updateOne({ organizationId: item.organizationId, eventId: item.payload.eventId }, { $set: { status: "processed", processedAt: new Date() } });
                        }
