 ...
 */
export async function verifyDemoTenant(request, _reply) {
    if (!request.demoUser) {
 ...
 */
export function requireDemoFeature(featureKey) {
    return async (request, _reply) => {
        ...
        if (!isEntitled) {
            ...
            throw new AppError(403, "DEMO_FEATURE_RESTRICTED", `This feature ('${featureKey}') is available in a full guided demonstration. Contact your sales representative for an executive walkthrough.`, {
                featureKey,
 ...
 */
export async function guardDemoHighRisk(request, _reply) {
    const DemoActivityLog = getDemoActivityLogModel();
    if (request.demoUser) {
        await DemoActivityLog.create({
            demoTenantId: request.demoUser.tenantId,
    ...
    throw new AppError(403, "DEMO_ACTION_RESTRICTED", "This administrative or high-risk action is restricted in the demo environment.");
}
