...
import { getDemoTenantModel, getDemoUserModel, getDemoSessionModel, getDemoActivityLogModel, getDemoRiskAlertModel, getDemoResetLogModel, getDemoEmailLogModel, getDemoJourneyModel, getDemoTaskModel, getDemoDocumentModel, getDemoKBArticleModel, getDemoMilestoneModel, getDemoFeatureUsageModel, } from "../models/index.js";
import { DETERMINISTIC_DEMO_COMPANIES, DETERMINISTIC_DEMO_USERS, DETERMINISTIC_DEMO_JOURNEYS, DETERMINISTIC_DEMO_TASKS, DETERMINISTIC_DEMO_DOCUMENTS, DETERMINISTIC_DEMO_KB_ARTICLES, DETERMINISTIC_DEMO_MILESTONES, } from "../seed/demo-seed-data.js";
...
export class DemoResetService {
     ...
     */
    async executeReset(performedBy = "Super Admin", actorRole = "super_admin") {
        if (isResettingLock) {
        ...
        try {
            ...
            for (const comp of DETERMINISTIC_DEMO_COMPANIES) {
                ...
                companyMap[comp.slug] = createdTenant._id;
            }
            const userDocs = [];
            ...
            const taskDocs = DETERMINISTIC_DEMO_TASKS.map((t, idx) => ({
                ...
                requiresVerification: t.priority === "critical",
                comments: [
                    {
                        userId: {
            ...
            const docEntries = DETERMINISTIC_DEMO_DOCUMENTS.map((d) => {
                ...
                return {
                    ...
                    content: d.content || "Standard company policy agreement for new employees.",
                    renderedContent: d.content || "Standard company policy agreement for new employees.",
     ...
     */
    static async ensureSeeded(force = false) {
        try {
            const DemoTenant = getDemoTenantModel();
            const DemoJourney = getDemoJourneyModel();
            const DemoKBArticle = getDemoKBArticleModel();
