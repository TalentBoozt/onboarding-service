...
import { getDemoUserModel, getDemoTenantModel, getDemoSessionModel, getDemoActivityLogModel, getDemoRiskAlertModel, } from "../models/index.js";
import { DemoResetService } from "./demo-reset.service.js";
export class DemoAuthService {
    /**
     * Generates a signed JWT specifically for the demo environment.
     */
    generateDemoToken(payload, jwtSigner) {
     ...
     */
    async login(email, password, jwtSigner, ipAddress, userAgent, deviceInfo) {
        if (DemoResetService.isResetting()) {
            throw new AppError(503, "DEMO_RESETTING", "The demo environment is currently undergoing an administrative reset. Please try again in a few moments.");
        }
        ...
        if (activeSessions.length >= tenant.sessionLimit) {
            ...
            const differentDeviceOrIp = activeSessions.some((s) => (s.ipAddress && ipAddress && s.ipAddress !== ipAddress) ||
            ...
            if (differentDeviceOrIp) {
                ...
                await DemoSession.updateMany({ demoUserId: user._id, isValid: true }, { isValid: false, suspiciousReason: "Superseded by new concurrent session" });
            }
            else {
                // Same IP/device refreshing session
                await DemoSession.updateMany({ demoUserId: user._id, isValid: true }, { isValid: false, suspiciousReason: "Re-authenticated on same device" });
            }
        }
     ...
     */
    async logout(sessionId, userId, tenantId) {
        const DemoSession = getDemoSessionModel();
        const DemoActivityLog = getDemoActivityLogModel();
        await DemoSession.updateOne({ sessionId }, { isValid: false, suspiciousReason: "User logged out explicitly" });
        if (userId && tenantId) {
            await DemoActivityLog.create({
