...
export class DemoSessionService {
     ...
     */
    async validateSession(sessionId, currentIp) {
        ...
        if (!session.isValid) {
            throw new AppError(401, "SESSION_REVOKED", session.suspiciousReason || "Demo session has been invalidated or terminated.");
        }
     ...
     */
    async terminateSession(sessionId, terminatedBy = "Super Admin") {
        const DemoSession = getDemoSessionModel();
        const DemoActivityLog = getDemoActivityLogModel();
        const session = await DemoSession.findOneAndUpdate({ sessionId }, { isValid: false, suspiciousReason: `Terminated by ${terminatedBy}` }, { new: true });
        if (!session) {
     ...
     */
    async listActiveSessions(demoTenantId) {
        const DemoSession = getDemoSessionModel();
     ...
     */
    async resolveRiskAlert(alertId, resolvedBy, resolutionNotes) {
        const DemoRiskAlert = getDemoRiskAlertModel();
        const alert = await DemoRiskAlert.findByIdAndUpdate(alertId, {
            status: "RESOLVED",
            resolvedBy,
            resolvedAt: new Date(),
            resolutionNotes,
