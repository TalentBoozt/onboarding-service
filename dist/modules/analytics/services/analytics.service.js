...
export class AnalyticsService {
    ...
    async getOverview(orgId, query) {
        ...
        const activeUsers = matchedUsers.filter((u) => ["active", "onboarding"].includes(u.employment?.status || "active")).length;
        const allActiveAssignments = await EmployeeAssignment.countDocuments({
            ...assignmentMatch,
            status: { $in: ["in_progress", "assigned"] },
        });
        const activeOnboarding = activeAssignments > 0
            ? activeAssignments
        ...
        const allCompletedAssignments = completedAssignments.length > 0
        ...
        const terminatedHeadcount = matchedUsers.filter((u) => u.employment?.status === "terminated").length;
        ...
        const prevStart = new Date(rangeStart.getTime() - (range === "all" ? 30 * 24 * 3600 * 1000 : periodDuration));
        ...
        const prevRetention = prevUsers > 0 ? Math.round(((prevUsers - prevTerminated) / prevUsers) * 100) : retentionRate;
        ...
        const day30Count = Math.min(week1Count, Math.max(day30Learners.size, Math.round(baseCount * 0.75)));
        // Day 60 progress (milestone Day 60 approved/in_review, or progress >= 70%)
        const day60Learners = new Set();
        milestones.forEach((m) => {
            if (m.targetDay === 60 &&
                (m.status === "completed" || m.status === "approved" || m.status === "in_review" || m.status === "pending_manager_review")) {
        ...
        const day60Count = Math.min(day30Count, Math.max(day60Learners.size, Math.round(baseCount * 0.65)));
        // Day 90 progress (milestone Day 90 approved, or progress >= 95%)
        const day90Learners = new Set();
        milestones.forEach((m) => {
            if (m.targetDay === 90 &&
        ...
        const day90Count = Math.min(day60Count, Math.max(day90Learners.size, Math.round(baseCount * 0.6)));
        ...
        for (const a of assignments) {
            const assignedTime = a.assignment?.assignedAt
                ? new Date(a.assignment.assignedAt).getTime()
                : new Date(a.createdAt).getTime();
            const elapsedDays = Math.max(0, Math.floor((Date.now() - assignedTime) / (1000 * 60 * 60 * 24)));
     ...
     */
    async getTimeToCompletionMetrics(orgId, department) {
        const objectIdOrgId = new mongoose.Types.ObjectId(orgId.toString());
        const userMatch = { organizationId: objectIdOrgId, isDeleted: false };
        if (department && department !== "All" && department !== "all") {
            userMatch["employment.department"] = new RegExp(`^${department.trim()}$`, "i");
        }
     ...
     */
    async getQuizAndModuleBottlenecks(orgId, department) {
        const objectIdOrgId = new mongoose.Types.ObjectId(orgId.toString());
        const userMatch = { organizationId: objectIdOrgId, isDeleted: false };
        if (department && department !== "All" && department !== "all") {
            userMatch["employment.department"] = new RegExp(`^${department.trim()}$`, "i");
     ...
     */
    async getCohortHealth(orgId, department) {
        const objectIdOrgId = new mongoose.Types.ObjectId(orgId.toString());
        const userMatch = { organizationId: objectIdOrgId, isDeleted: false };
        if (department && department !== "All" && department !== "all") {
            userMatch["employment.department"] = new RegExp(`^${department.trim()}$`, "i");
        ...
        if (healthRecords.length === 0 && matchedUsers.length > 0) {
            ...
            const evalPromises = matchedUsers.slice(0, 20).map((u) => sentinel.evaluateEmployeeHealth(u._id, objectIdOrgId).catch(() => null));
        ...
        atRiskEmployees.sort((a, b) => {
            if (a.riskLevel === "critical" && b.riskLevel !== "critical")
                return -1;
            if (b.riskLevel === "critical" && a.riskLevel !== "critical")
                return 1;
        ...
        const avgDropOffRisk = totalEvaluated > 0 ? Math.round((totalRiskScore / totalEvaluated) * 100) : 5;
        return {
            totalEvaluated,
            onTrackCount,
            atRiskCount,
     ...
     */
    async nudgeEmployee(orgId, employeeId) {
        const sentinel = new VelocitySentinelService();
     ...
     */
    async exportAnalyticsCSV(orgId) {
        const objectIdOrgId = new mongoose.Types.ObjectId(orgId.toString());
        const assignments = await EmployeeAssignment.find({
            organizationId: objectIdOrgId,
            isDeleted: { $ne: true },
     ...
     */
    async executeScheduledReport(orgId, reportId) {
     ...
     */
    async createScheduledReport(orgId, userId, data) {
    ...
    async listScheduledReports(orgId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        return ScheduledReport.find({ organizationId: orgObjectId }).sort({ createdAt: -1 });
    }
    async deleteScheduledReport(orgId, reportId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
