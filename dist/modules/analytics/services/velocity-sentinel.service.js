...
export class VelocitySentinelService {
     ...
     */
    async evaluateEmployeeHealth(userId, orgId, options) {
        ...
        // Item Metrics
        const totalLessons = assignments.reduce((sum, a) => sum + (a.progress?.totalLessons || 0), 0);
        const completedLessons = assignments.reduce((sum, a) => sum + (a.progress?.completedLessons || 0), 0);
        ...
        const overdueTasks = tasks.filter((t) => t.dueDate &&
            ...
            t.status !== "completed" &&
            t.status !== "verified").length;
        const totalDocs = documents.length;
        const completedDocs = documents.filter((d) => d.status === "signed").length;
        const overdueDocs = documents.filter((d) => d.dueDate && new Date(d.dueDate) < now && d.status !== "signed").length;
        const overdueAssignments = assignments.filter((a) => a.assignment?.dueDate &&
        ...
        assignments.forEach((a) => {
            if (a.progress?.lastActivityAt)
                activityDates.push(new Date(a.progress.lastActivityAt));
            if (a.updatedAt)
                activityDates.push(new Date(a.updatedAt));
        });
        tasks.forEach((t) => {
            if (t.completedAt)
                activityDates.push(new Date(t.completedAt));
        ...
        documents.forEach((d) => {
            if (d.signedAt)
                activityDates.push(new Date(d.signedAt));
        ...
        const daysInactive = Math.max(0, Math.floor((now.getTime() - lastActiveAt.getTime()) / (1000 * 60 * 60 * 24)));
        // Velocity & Expected Velocity Calculation
        const startDate = user.employment?.hireDate || user.createdAt || now;
        const elapsedDays = Math.max(1, Math.floor((now.getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)));
        ...
        const riskLevel = dropOffRiskScore >= 0.85
            ? "critical"
        ...
        const hasUrgentDueItem = tasks.some((t) => t.dueDate &&
            new Date(t.dueDate) > now &&
        ...
        if (desiredLevel === 1) {
            ...
            await OnboardingCase.updateOne({ employeeId: user._id, organizationId }, {
                $set: {
     ...
     */
    async scanOrganizationHealth(orgId, options) {
        ...
        for (const candidate of candidates) {
            ...
            if (res) {
                ...
                if (res.nudged)
                    nudgedCount++;
                if (res.health.riskLevel === "critical")
                    criticalCount++;
                if (res.health.suppressedReason)
                    suppressedCount++;
            }
        }
        return {
     ...
     */
    async scanAllOrganizations(options) {
