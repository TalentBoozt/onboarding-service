...
const pwaProgressState = {};
export class EmployeeAssignmentController {
    service;
    constructor(service) {
        this.service = service;
    }
    getAssignment = async (request, reply) => {
    ...
    listAssignments = async (request, reply) => {
        const user = request.user;
    ...
    getMyAssignments = async (request, reply) => {
        const user = request.user;
        const query = request.query;
        const filter = {
            organizationId: user.organizationId,
    ...
    getMyActiveAssignments = async (request, reply) => {
        const user = request.user;
        const query = request.query;
    ...
    getMyCompletedAssignments = async (request, reply) => {
        const user = request.user;
        const query = request.query;
        const filter = {
            organizationId: user.organizationId,
    ...
    assignJourney = async (request, reply) => {
        ...
        const assignment = await this.service.assignJourney(user.organizationId, body.employeeId, body.journeyId, user.userId, {
    ...
    bulkAssignJourneys = async (request, reply) => {
        ...
        const result = await this.service.bulkAssignJourneys(user.organizationId, body.employeeIds, body.journeyId, user.userId, {
            dueDate: body.dueDate,
            priority: body.priority,
        });
        return reply.status(201).send({
    ...
    startAssignment = async (request, reply) => {
        const user = request.user;
        const params = request.params;
    ...
    completeLesson = async (request, reply) => {
        const user = request.user;
        const params = request.params;
        const body = request.body;
        const assignment = await this.service.completeLesson(params.id, user.organizationId, body.moduleId, body.lessonId, body.timeSpentSeconds, body.completedBlockIds, user.userId, user.role);
        return reply.status(200).send({
            success: true,
    ...
    updateProgress = async (request, reply) => {
        ...
        if (params.id === "assign-pwa-01") {
            ...
            const incoming = body.batchCompletions
                ? body.batchCompletions.map((b) => b.lessonId)
        ...
        if (body.batchCompletions && Array.isArray(body.batchCompletions)) {
            const result = await this.service.reconcileOfflineProgress(params.id, user.organizationId, user.userId, body.batchCompletions, user.role);
            return reply.status(200).send({
                success: true,
        ...
        if (body.completedLessonIds && Array.isArray(body.completedLessonIds)) {
            ...
            for (const lid of body.completedLessonIds) {
                for (const mod of assignment.modules) {
                    const les = mod.lessons.find((l) => l.lessonId.toString() === lid.toString());
                    if (les) {
        ...
        if (body.moduleId && body.lessonId) {
            const assignment = await this.service.completeLesson(params.id, user.organizationId, body.moduleId, body.lessonId, body.timeSpentSeconds || 0, body.completedBlockIds || []);
            return reply.status(200).send({
                success: true,
                message: "Assignment progress updated successfully",
                data: assignment,
    ...
    submitQuiz = async (request, reply) => {
        const user = request.user;
        const params = request.params;
        const body = request.body || {};
        ...
        const result = await this.service.submitQuiz(params.id, user.organizationId, body.moduleId, body.lessonId, body.answers);
        return reply.status(200).send({
            success: true,
    ...
    verifyCertificatePublic = async (request, reply) => {
        const params = request.params;
        try {
            let assignment = null;
            ...
            if (!assignment || assignment.status !== "completed" || !assignment.certificate?.issued) {
                ...
                const idQueries = [{ certificateNumber: params.id }];
                if (mongoose.Types.ObjectId.isValid(params.id)) {
                ...
                const cert = await Certificate.findOne({ $or: idQueries });
                if (cert) {
    ...
    issueCertificate = async (request, reply) => {
        const user = request.user;
        const params = request.params;
