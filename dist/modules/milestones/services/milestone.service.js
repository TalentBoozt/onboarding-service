...
import MilestoneTemplate from "../models/milestone-template.model.js";
import EmployeeMilestone from "../models/employee-milestone.model.js";
...
export class MilestoneService {
     ...
     */
    async updateTemplate(orgId, templateId, userId, data) {
        ...
        if (data.targetDay !== undefined)
            template.targetDay = data.targetDay;
        if (data.goals !== undefined)
            template.goals = data.goals;
        if (data.checkinQuestions !== undefined)
            template.checkinQuestions = data.checkinQuestions;
     ...
     */
    async assignMilestone(orgId, templateId, employeeId, assignedByUserId) {
        const template = await MilestoneTemplate.findOne({
            _id: new mongoose.Types.ObjectId(templateId),
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
     ...
     */
    async getEmployeeMilestones(orgId, employeeId) {
     ...
     */
    async getManagerTeamMilestones(orgId, managerUserId, role) {
     ...
     */
    async submitEmployeeSelfCheck(orgId, milestoneId, employeeId, payload) {
        let filter = {
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        };
        ...
        const blockersReported = lower.includes("block") ||
     ...
     */
    async evaluateMilestone(orgId, milestoneId, managerUserId, role, payload) {
        let filter = {
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        };
        ...
        if (!isRevision) {
            ...
            try {
                await eventBus.publish({
                    eventName: "ON_MILESTONE_EVALUATED",
                    organizationId: orgId,
     ...
     */
    async submitManagerReview(orgId, milestoneId, managerUserId, role, payload) {
     ...
     */
    async getMilestone(orgId, milestoneId, userId, role) {
        let filter = {
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        };
        ...
        const targetEmpId = milestone.employeeId?._id?.toString() || milestone.employeeId?.toString();
     ...
     */
    async updateMilestoneStatus(orgId, milestoneId, actorUserId, role, payload) {
        let filter = {
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        };
     ...
     */
    async updateMilestoneGoals(orgId, milestoneId, userId, role, goalsProgress) {
        let filter = {
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        };
     ...
     */
    async autoAssignMilestonesToNewHire(orgId, userId) {
     ...
     */
    async scanPendingMilestoneReviews(orgId) {
        const filter = {
        ...
        for (const milestone of pendingMilestones) {
            ...
            const reviewDeadline = milestone.sla?.reviewDeadline ||
            ...
            if (milestone.sla?.escalationState === "auto_approved" ||
            ...
            if (isBreached) {
                ...
                if (milestone.sla?.autoApprovalEligible) {
                    ...
                    try {
                        ...
                        await eventBus.publish({
                            eventName: "milestone.auto_approved",
                            organizationId: milestone.organizationId,
                            actorId: undefined,
                            entityId: milestone._id,
                            payload: {
