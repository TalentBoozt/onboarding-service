...
export class OrgEmailService {
     ...
     */
    async testConnection(config, secrets, targetEmail) {
        ...
        try {
            if (config.provider === "smtp") {
                ...
                const transportOptions = {
                    ...
                    auth: config.user && secrets.password
     ...
     */
    async sendEmail(config, secrets, to, subject, html) {
        if (process.env.NODE_ENV === "test") {
     ...
     */
    async sendInvitationEmail(config, secrets, to, token, orgName) {
        EmailService.sentEmails.push({ to, token });
     ...
     */
    async sendPasswordResetEmail(config, secrets, to, token) {
        EmailService.sentEmails.push({ to, token });
