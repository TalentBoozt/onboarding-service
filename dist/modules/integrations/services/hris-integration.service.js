...
export class HRISIntegrationService {
     ...
     */
    async createIntegration(orgId, userId, data) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
     ...
     */
    async connectProvider(orgId, userId, provider, data) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const userObjectId = new mongoose.Types.ObjectId(userId.toString());
        ...
        if (data.subdomain)
            integration.subdomain = data.subdomain.trim();
        if (data.apiSecret)
            integration.apiSecret = data.apiSecret.trim();
     ...
     * Disconnect Provider
     */
    async disconnectProvider(orgId, provider) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
     ...
     */
    async syncProvider(orgId, provider, records) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const integration = await HRISIntegration.findOne({
            organizationId: orgObjectId,
            provider: provider.toLowerCase(),
     ...
     */
    async updateIntegration(orgId, integrationId, data) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const integration = await HRISIntegration.findOne({
        ...
        if (data.subdomain)
            integration.subdomain = data.subdomain;
     ...
     */
    async testConnection(orgId, integrationId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const integration = await HRISIntegration.findOne({
            _id: new mongoose.Types.ObjectId(integrationId),
            organizationId: orgObjectId,
     ...
     */
    async triggerSync(orgId, integrationId, incomingRecords) {
        ...
        for (const rawRecord of recordsToSync) {
            try {
                ...
                const isTerminated = rawRecord.status === "terminated" ||
                ...
                if (!user) {
                    ...
                    if (integration.conflictPolicy === "hris_wins") {
                        if (mappedData.firstName)
                            user.profile.firstName = mappedData.firstName;
                        if (mappedData.lastName)
                            user.profile.lastName = mappedData.lastName;
                        if (mappedData.department)
     ...
     */
    async processWebhookPayload(provider, signature, payload) {
        ...
        const syncResult = await this.triggerSync(integration.organizationId, integration._id.toString(), records);
        return {
            success: true,
            eventId,
            eventType,
     ...
     */
    async executeTerminationLifecycle(orgId, employeeIdOrEmail, reason = "hris_termination_event") {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const isId = mongoose.Types.ObjectId.isValid(employeeIdOrEmail) &&
        ...
        await taskService.cancelTasksForTerminatedEmployee(orgObjectId, user._id, "Cancelled due to HRIS termination event");
     ...
     */
    async getSyncLogs(orgId, integrationId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const query = { organizationId: orgObjectId };
        if (integrationId) {
     ...
     */
    async rotateWebhookSecret(orgId, integrationId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const integration = await HRISIntegration.findOne({
            _id: new mongoose.Types.ObjectId(integrationId),
            organizationId: orgObjectId,
     ...
     */
    async retryDLQEvent(orgId, integrationId, eventId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        ...
        const targetDlq = syncLog.dlqEvents.find((e) => e.eventId === eventId);
        if (!targetDlq) {
