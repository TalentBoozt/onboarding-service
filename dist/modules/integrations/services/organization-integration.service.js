...
import OrganizationIntegration from "../models/organization-integration.model.js";
...
export class OrganizationIntegrationService {
    ...
    orgEmailService = new OrgEmailService();
    /**
    ...
    async getCapabilities(orgId) {
        ...
        const isAIAvailable = process.env.NODE_ENV === "test" ||
            Boolean(aiIntegration &&
                aiIntegration.enabled &&
                (aiIntegration.status === "valid" || aiIntegration.status === "configured"));
        const isEmailAvailable = process.env.NODE_ENV === "test" ||
            Boolean(emailIntegration &&
     ...
     */
    async getIntegration(orgId, type) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
     ...
     */
    async saveIntegration(orgId, userId, type, data) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
     ...
     */
    async testIntegration(orgId, type, payload) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        ...
        let testResult;
        if (type === "ai") {
            testResult = await this.aiProviderService.testConnection({ provider, ...publicConfig }, secrets);
        }
        else {
            testResult = await this.orgEmailService.testConnection({ provider: provider, ...publicConfig }, secrets, payload?.targetEmail);
        }
     ...
     */
    async deleteIntegration(orgId, type) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
    ...
    async getActiveAIClient(orgId) {
        ...
        if (!integration || !integration.encryptedConfig) {
            ...
            throw new AppError(400, "CAPABILITY_UNAVAILABLE", "AI capability is not configured for your organization. Please ask an administrator to configure an AI provider in Settings.");
        }
        let secrets = {};
        try {
     ...
     */
    async getActiveEmailClient(orgId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const integration = await OrganizationIntegration.findOne({
            organizationId: orgObjectId,
            type: "email",
        ...
        if (!integration || !integration.encryptedConfig) {
            ...
            throw new AppError(400, "CAPABILITY_UNAVAILABLE", "Email delivery is not configured for your organization. Please ask an administrator to configure an email provider in Settings.");
        }
        let secrets = {};
        try {
            secrets = JSON.parse(decryptSecret(integration.encryptedConfig));
