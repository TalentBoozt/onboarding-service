import AIUsageRecord from "../../super-admin/models/ai-usage-record.model.js";
...
export function calculateAICostUsd(model, inputTokens, outputTokens) {
...
export class AIProviderService {
    ...
    static async recordUsage(params) {
        try {
            const inputTokens = Math.max(0, Number(params.inputTokens !== undefined ? params.inputTokens : params.promptTokens) || 0);
            const outputTokens = Math.max(0, Number(params.outputTokens !== undefined ? params.outputTokens : params.completionTokens) || 0);
            const latencyMs = Math.max(0, Number(params.latencyMs !== undefined ? params.latencyMs : params.durationMs) || 0);
     ...
     */
    async chatCompletion(config, secrets, messages, context) {
     ...
     */
    async generateCourseCurriculum(config, secrets, prompt, targetRole, department, level, moduleCount, groundedArticles, context) {
        const articlesContext = groundedArticles
            .map((art, idx) => `[Document ${idx + 1}: ${art.title}]\nSummary: ${art.summary || "N/A"}\nKey points: ${art.content?.blocks?.map((b) => b.content).filter(Boolean).join(" ").slice(0, 300) || ""}`)
            .join("\n\n");
