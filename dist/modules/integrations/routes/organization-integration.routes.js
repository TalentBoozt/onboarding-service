...
export async function organizationIntegrationRoutes(app) {
    ...
    app.get("/capabilities", controller.getCapabilities);
    ...
    app.get("/:type", adminHook, controller.getIntegration);
    // PUT /api/v1/organizations/integrations/:type (ai | email)
    app.put("/:type", adminHook, controller.saveIntegration);
    // POST /api/v1/organizations/integrations/:type/test (ai | email)
    app.post("/:type/test", adminHook, controller.testIntegration);
    // DELETE /api/v1/organizations/integrations/:type (ai | email)
    app.delete("/:type", adminHook, controller.deleteIntegration);
}
