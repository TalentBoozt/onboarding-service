...
export class GamificationService {
     ...
     */
    async awardPoints(orgId, userId, action, points, description, referenceId) {
     ...
     */
    async recordActivityStreak(orgId, userId) {
        const profile = await this.getProfile(orgId, userId);
        await this.recordActivityStreakInternal(profile);
        await this.checkAndUnlockBadgesInternal(profile);
        profile.markModified("unlockedBadges");
    ...
    async recordActivityStreakInternal(profile) {
        const now = new Date();
     ...
     */
    async checkAndUnlockBadgesInternal(profile) {
        for (const b of AVAILABLE_BADGES) {
            const alreadyUnlocked = profile.unlockedBadges.some((ub) => ub.badgeId === b.badgeId);
            if (alreadyUnlocked)
                continue;
            let unlock = false;
            if (b.actionRequired && profile.pointHistory.some((ph) => ph.action === b.actionRequired)) {
                unlock = true;
            }
            if (b.pointsRequired && profile.points >= b.pointsRequired)
                unlock = true;
            if (b.levelRequired && profile.level >= b.levelRequired)
                unlock = true;
            if (b.streakRequired && profile.currentStreak >= b.streakRequired)
                unlock = true;
            if (unlock) {
     ...
     */
    async getLeaderboard(orgId, limit = 20) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
