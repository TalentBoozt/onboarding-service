...
export class OfficeLocationService {
     ...
     */
    async getLocationById(orgId, locationId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
     ...
     */
    async updateLocation(orgId, locationId, data) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        ...
        if (data.isPrimary !== undefined)
            location.isPrimary = data.isPrimary;
        await location.save();
        return location;
    }
    /**
     ...
     */
    async assignEmployeeDesk(orgId, locationId, floorNumber, deskNumber, targetUserId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
    ...
    async getEmployeeLocationGuidance(orgId, userId) {
        ...
        const encodedAddress = encodeURIComponent(`${location.address.street}, ${location.address.city}, ${location.address.country}`);
        return {
