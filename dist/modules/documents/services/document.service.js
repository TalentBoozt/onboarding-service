...
import DocumentTemplate from "../models/document-template.model.js";
import DocumentAssignment from "../models/document-assignment.model.js";
...
export class DocumentService {
    ...
    async updateTemplate(orgId, templateId, userId, data) {
        ...
        if (data.signatureRequired !== undefined)
            template.signatureRequired = data.signatureRequired;
        if (data.isMandatory !== undefined)
            template.isMandatory = data.isMandatory;
        if (data.audience)
            template.audience = { ...template.audience, ...data.audience };
     ...
     */
    interpolateContent(content, user, orgName) {
     ...
     */
    async assignDocument(orgId, templateId, employeeId, assignedByUserId, dueDate) {
        const template = await this.getTemplate(orgId, templateId);
     ...
     */
    async getEmployeeDocumentInbox(orgId, employeeId) {
     ...
     */
    async getDocumentAssignment(orgId, assignmentId, userId, role, reqMetadata) {
     ...
     */
    async signDocument(orgId, assignmentId, employeeId, payload, reqMetadata, userRole) {
     ...
     */
    async getTemplateSignatures(orgId, templateId) {
        return DocumentAssignment.find({
            organizationId: new mongoose.Types.ObjectId(orgId),
     ...
     */
    async autoAssignDocumentsToNewHire(orgId, userId) {
     ...
     */
    async handleEmployeeTermination(orgId, employeeId) {
