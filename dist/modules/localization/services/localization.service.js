import { TranslationRepository, MemoryCacheProvider, LocaleNegotiator, TranslationVersionManager, NoOpEventBus, } from "../../../infrastructure/localization/index.js";
import { SUPPORTED_LOCALES, DEFAULT_LOCALE, TRANSLATABLE_FIELDS, } from "../../../config/localization.config.js";
...
export class LocalizationService {
     ...
     */
    async translateRealtime(text, targetLanguage) {
    ...
    validateLocale(lang) {
        ...
        if (!resolved) {
            throw new AppError(400, "UNSUPPORTED_LOCALE", `Locale '${lang}' is not supported. Supported: ${SUPPORTED_LOCALES.join(", ")}`);
        }
        return resolved;
    }
    /**
     ...
     */
    negotiateLocale(acceptLanguageHeader) {
     ...
     */
    async getTranslation(entityType, entityId, field, requestedLang) {
    ...
    async _fetchCached(entityType, entityId, field, language) {
     ...
     */
    async localizeDocument(entityType, doc, locale, fields) {
        if (locale === DEFAULT_LOCALE)
            return doc; // English is already the source
        try {
     ...
     */
    async upsertTranslation(entityType, entityId, field, language, data) {
        const id = entityId.toString();
     ...
     */
    async approveTranslation(entityType, entityId, field, language, approverId) {
        const id = entityId.toString();
     ...
     */
    async listTranslations(entityType, entityId) {
     ...
     */
    async listMissingTranslations(entityType, language) {
     ...
     */
    async listOutdatedTranslations(entityType) {
     ...
     */
    async translationCoverage(entityType) {
