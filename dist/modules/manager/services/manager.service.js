...
export class ManagerService {
     ...
     */
    buildDirectReportQuery(orgId, managerUserId, role) {
     ...
     */
    async getManagerDashboard(orgId, managerUserId, role) {
        ...
        const activeOnboardingCount = directReports.filter((u) => u.employment?.status === "onboarding" || u.employment?.status === "active").length;
        let totalCompletionPercentage = 0;
        assignments.forEach((a) => {
            totalCompletionPercentage += a.progress?.completionPercentage || 0;
        });
        const overallCompletionRate = assignments.length > 0 ? Math.round(totalCompletionPercentage / assignments.length) : 0;
        const overdueAssignments = assignments.filter((a) => a.status === "overdue" || (a.assignment.dueDate && new Date(a.assignment.dueDate) < new Date() && a.status !== "completed")).length;
     ...
     */
    async getTeamDirectReports(orgId, managerUserId, role) {
        const userQuery = this.buildDirectReportQuery(orgId, managerUserId, role);
        ...
        return directReports.map((user) => {
            ...
            const empTasks = tasks.filter((t) => t.assignedToUserId?.toString() === empIdStr ||
                t.employeeId?.toString() === empIdStr ||
                t.assigneeId?.toString() === empIdStr);
            ...
            const overdueTasks = empTasks.filter((t) => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== "completed").length;
            const overdueJourneys = empAssignments.filter((a) => a.status === "overdue" || (a.assignment.dueDate && new Date(a.assignment.dueDate) < new Date() && a.status !== "completed")).length;
     ...
     */
    async getDirectReportDetails(orgId, managerUserId, role, employeeId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        ...
        if (role === "manager") {
            ...
            const isDirectReport = employee.employment?.managerId?.toString() === mgrIdStr ||
                employee.employment?.managerUserId?.toString() === mgrIdStr;
            if (!isDirectReport) {
     ...
     */
    async nudgeDirectReport(orgId, managerUserId, role, employeeId, customMessage) {
     ...
     */
    async signOffDirectReport(orgId, managerUserId, role, employeeId, notes) {
        const details = await this.getDirectReportDetails(orgId, managerUserId, role, employeeId);
        // Update user employment status to active upon manager sign-off
        await User.updateOne({ _id: new mongoose.Types.ObjectId(employeeId), organizationId: new mongoose.Types.ObjectId(orgId) }, {
            $set: {
