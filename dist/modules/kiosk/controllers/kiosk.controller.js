...
export class KioskController {
    ...
    getJourney = async (request, reply) => {
        const orgId = request.kioskContext?.organizationId || request.user?.organizationId;
        if (!orgId) {
        ...
        FeatureTelemetryService.recordUsage({
            ...
            userRole: request.user?.role || "frontline_worker_kiosk",
    ...
    getSession = async (request, reply) => {
        ...
        if (!user && request.headers.authorization) {
            try {
                ...
                user = request.server.jwt.decode(token);
            }
            catch {
                // ignore
            }
        }
        const orgId = user?.organizationId || request.kioskContext?.organizationId || request.query?.organizationId;
        const params = request.params;
        let session = null;
        try {
    ...
    listJourneys = async (request, reply) => {
        const user = request.user;
    ...
    // --- Device Management ---
    generatePairingCode = async (request, reply) => {
        const user = request.user;
    ...
    pairDevice = async (request, reply) => {
        const body = request.body;
        ...
        return reply.status(200).send({
            ...
            device: {
                ...
                hardwareGuid: result.device.hardwareGuid || result.device.deviceId,
                location: result.device.location,
                status: result.device.status,
                paired: result.device.paired ?? true
            },
            data: {
                deviceToken: result.token,
                token: result.token,
    ...
    heartbeat = async (request, reply) => {
        const devicePayload = request.user;
        const body = (request.body || {});
        const telemetry = {
            ...(body.telemetry || {}),
        ...
        const updated = await this.kioskService.heartbeat(devicePayload.deviceId, devicePayload.organizationId, body.currentContentVersion || body.contentVersion || 0, telemetry);
        return reply.status(200).send({
            success: true,
            status: "ok",
    ...
    pairJourneyToDevice = async (request, reply) => {
        const user = request.user;
        const params = request.params;
        const body = request.body;
        const updated = await this.kioskService.pairJourneyToDevice(params.id, user.organizationId, body.journeyId || null);
        return reply.status(200).send({
            success: true,
    ...
    // --- Player & Security API ---
    validatePIN = async (request, reply) => {
        const params = request.params;
        const body = request.body;
        if (!body?.pinCode) {
    ...
    syncAnalytics = async (request, reply) => {
    ...
    getJourneyAnalyticsSummary = async (request, reply) => {
        const user = request.user;
        const params = request.params;
        const query = request.query;
        const summary = await this.kioskService.getJourneyAnalyticsSummary(params.id, user.organizationId, query.startDate, query.endDate);
        return reply.status(200).send({
            success: true,
    ...
    identifyFrontlineWorker = async (request, reply) => {
        const body = request.body || {};
        let orgId = body.organizationId || request.user?.organizationId || request.headers["x-organization-id"];
        ...
        const result = await this.kioskService.identifyFrontlineWorker(orgId, identifier, body.kioskDeviceId);
        return reply.status(200).send({
            success: true,
    ...
    verifySupervisorPin = async (request, reply) => {
        const body = request.body || {};
        let orgId = body.organizationId || request.user?.organizationId || request.headers["x-organization-id"];
        ...
        const result = await this.kioskService.verifySupervisorPin(orgId, supId, body.pin);
        return reply.status(200).send({
            success: true,
    ...
    setSupervisorPin = async (request, reply) => {
        const user = request.user;
        const body = request.body || {};
        const targetUserId = body.supervisorId || user.userId;
        const result = await this.kioskService.setSupervisorPin(user.organizationId, targetUserId, body.pin);
        return reply.status(200).send({
            success: true,
    ...
    toggleMaintenanceMode = async (request, reply) => {
        const user = request.user;
        const params = request.params;
        const body = request.body || {};
        const isMaintenance = body.maintenance !== undefined ? Boolean(body.maintenance) : true;
        const device = await this.kioskService.setDeviceMaintenanceMode(params.id, user.organizationId, isMaintenance);
        return reply.status(200).send({
            success: true,
