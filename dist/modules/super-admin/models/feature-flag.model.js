...
FeatureFlagSchema.methods.isOrgTargeted = function (orgId) {
    const orgStr = orgId.toString();
    // 1. Excluded takes strict precedence (deny dominates)
    if (this.excludedOrganizationIds?.some((id) => (id._id || id).toString() === orgStr)) {
        return false;
    }
    // 2. Targeted overrides allow specific access even if global is disabled
    if (this.targetOrganizationIds?.some((id) => (id._id || id).toString() === orgStr)) {
        return true;
    }
