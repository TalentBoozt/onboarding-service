...
AIUsageRecordSchema.pre("validate", function (next) {
    ...
    if (this.inputTokens === undefined || this.inputTokens === 0) {
        if (doc.promptTokens !== undefined)
            this.inputTokens = Number(doc.promptTokens) || 0;
    }
    if (this.outputTokens === undefined || this.outputTokens === 0) {
        if (doc.completionTokens !== undefined)
            this.outputTokens = Number(doc.completionTokens) || 0;
    }
    if (this.latencyMs === undefined || this.latencyMs === 0) {
        if (doc.durationMs !== undefined)
            this.latencyMs = Number(doc.durationMs) || 0;
    }
    if (this.estimatedCostUsd === undefined || this.estimatedCostUsd === 0) {
        if (doc.costEstimateUSD !== undefined)
            this.estimatedCostUsd = Number(doc.costEstimateUSD) || 0;
    }
...
export const AIUsageRecord = mongoose.models.AIUsageRecord ||
    mongoose.model("AIUsageRecord", AIUsageRecordSchema);
