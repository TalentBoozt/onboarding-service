...
export class AlertService {
            ...
            for (const t of overdueTasks) {
                ...
                if (!existing) {
                    ...
                    await Alert.create({
                        ...
                        sourceId,
                        status: "open",
                        createdAt: t.dueDate || t.createdAt || new Date(),
                    }).catch(() => { });
                }
            }
        }
     ...
     */
    static async updateAlertStatus(params) {
        ...
        if (alert.status === "resolved" && status === "resolved") {
            const err = new Error("Alert is already resolved");
            err.statusCode = 400;
            throw err;
        }
        ...
        if (operatorUserId && mongoose.Types.ObjectId.isValid(operatorUserId)) {
            await AuditLog.create({
                ...
                description: `Alert ${alert.alertNo} status changed from ${previousStatus} to ${status}${resolutionNotes ? `: ${resolutionNotes}` : ""}`,
