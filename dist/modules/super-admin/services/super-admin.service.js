...
import { DEFAULT_PLATFORM_FLAGS } from "../config/default-feature-flags.js";
export { DEFAULT_PLATFORM_FLAGS };
export class SuperAdminService {
    // ---------------------------------------------------------------------------
    // 1. Search & Telemetry
    // ---------------------------------------------------------------------------
    async searchGlobal(q, limit = "10") {
        ...
        const [orgs, users, journeys, invoices] = await Promise.all([
            ...
            User.find({
                ...
                .limit(limitNum)
                .select("_id profile.fullName auth.email permissions.role employment.status employment.department organizationId"),
            Journey.find({
                isDeleted: false,
            ...
            Invoice.find({
                ...
                .limit(limitNum)
                .select("_id invoiceNo customerName organization amount totalAmount balanceDue status dueDate organizationId"),
        ]);
        return {
            ...
            invoices: invoices.map((i) => ({
                ...
                customerName: i.customerName || i.organization,
    ...
    async getTelemetry(query) {
        ...
        const userFilter = { isDeleted: false, ...orgFilter };
        ...
        const expenseFilter = { isDeleted: { $ne: true } };
        ...
        const auditFilter = orgFilter.organizationId
        ...
        const systemHealthStatus = systemHealthVal >= 98.0
            ? "HEALTHY"
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const monthlyRevenue = monthlyPaidInvoices.reduce((sum, inv) => sum + (inv.totalAmount ?? inv.amount ?? 0), 0);
    ...
    async createOrganization(body, actorUserId) {
        const { name, domain, adminEmail, ownerEmail, email, slug, plan, tier, supportEmail, } = body || {};
        if (!name || !name.trim()) {
    ...
    async updateOrganizationStatus(id, status, actorUserId) {
        ...
        const updated = await Organization.findOneAndUpdate(filter, { $set: { status } }, { new: true });
        if (!updated) {
            throw new AppError(404, "NOT_FOUND", "Organization not found");
        }
        if (status === "Suspended") {
            await Session.updateMany({ organizationId: updated._id, isValid: true }, { $set: { isValid: false, revokedReason: "ORGANIZATION_QUARANTINED" } });
    ...
    async updateOrganization(id, body, actorUserId) {
        const { name, plan, status, seatLimit, seatQuota, maxUsers, subscription, } = body || {};
        const requestedSeats = seatLimit !== undefined
            ? seatLimit
            : seatQuota !== undefined
                ? seatQuota
                : maxUsers !== undefined
        ...
        const updated = await Organization.findByIdAndUpdate(org._id, { $set: updates }, { new: true });
        if (!updated) {
    ...
    async getOrganization360(id) {
        ...
        const orgId = org._id;
        const [usersCount, activeUsersCount, topUsers, journeys, invoices, activityLogs, activeOnboardings, completedOnboardings, storageAgg,] = await Promise.all([
        ...
        return {
            ...
            quotas: {
                ...
                storage: {
                    ...
                    utilizationPct: maxStorageBytes > 0
    ...
    async quarantineOrganization(id, reason, actorUserId) {
        const isObjId = mongoose.Types.ObjectId.isValid(id);
        const filter = isObjId
            ? { _id: id, isDeleted: false }
            : { slug: id.toLowerCase().trim(), isDeleted: false };
        const org = await Organization.findOneAndUpdate(filter, { $set: { status: "Suspended" } }, { new: true });
        if (!org) {
            throw new AppError(404, "NOT_FOUND", "Organization not found");
        }
        // Invalidate all active sessions for this quarantined tenant immediately
        await Session.updateMany({ organizationId: org._id, isValid: true }, { $set: { isValid: false, revokedReason: "ORGANIZATION_QUARANTINED" } });
    ...
    async getUsers(query) {
        const { search, organizationId, role, status, page = "1", limit = "15", } = query || {};
        ...
        return {
            users: users.map((u) => ({
                ...
                organization: u.organizationId
                    ? {
                        id: u.organizationId._id?.toString(),
    ...
    async getUser360(id) {
        ...
        const user = await User.findOne({ _id: id, isDeleted: false }).populate("organizationId", "name slug domain plan status");
        if (!user) {
    ...
    async updateUser(id, body, actorUserId) {
    ...
    async forceLogoutUser(id, actorUserId) {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            throw new AppError(400, "BAD_REQUEST", "Invalid User ID");
        }
        const user = await User.findOne({ _id: id, isDeleted: false });
            ...
            sessions: sessions.map((s) => ({
                ...
                user: s.userId
                    ? {
                        ...
                        name: s.userId.profile?.fullName || "User",
                        email: s.userId.auth?.email,
                        role: s.userId.permissions?.role,
                    }
                    : null,
                organization: s.organizationId
                    ? {
                        id: s.organizationId._id?.toString(),
                        name: s.organizationId.name,
                        slug: s.organizationId.slug,
                    }
                    : null,
    ...
    async revokeSession(sessionId, actorUserId) {
        ...
        const session = await Session.findOneAndUpdate({ _id: sessionId, isValid: true }, { $set: { isValid: false } }, { new: true });
        if (!session) {
    ...
    async getInvoices(query) {
        const { search, status, organizationId, page = "1", limit = "10", } = query || {};
        const pageNum = Math.max(1, parseInt(page) || 1);
        ...
        return {
            ...
            payments: payments.map((p) => ({
                ...
                referenceNumber: p.referenceNumber ||
                    ...
                    p._doc?.reference,
                notes: p.notes,
                recordedAt: p.paymentDate ||
                    p.recordedAt ||
                    p._doc?.recordedAt,
                paymentDate: p.paymentDate,
                recordedBy: p.recordedByEmail || p.recordedBy,
            })),
        };
    }
    async createInvoice(body, actorUserId) {
        const { organizationId, organization, customerName, currency = "USD", issueDate, dueDate, lineItems, discountAmount = 0, taxAmount = 0, amount, type = "Invoice", status = "issued", notes, description, } = body || {};
        ...
        if (Array.isArray(lineItems) && lineItems.length > 0) {
            preparedLineItems = lineItems.map((item) => {
    ...
    async getFinanceOverview() {
        ...
        for (const org of activeOrgs) {
            const rawPlan = (org.plan ||
                ...
                "Starter");
            const normalizedPlan = rawPlan === "Professional" ||
                rawPlan === "Growth" ||
                rawPlan === "Pro"
                ? "Pro"
                : rawPlan === "Enterprise"
                    ? "Enterprise"
                    : "Starter";
            const monthlyPrice = org.subscription?.price ??
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const invoiceRevenue = monthInvoices.reduce((sum, inv) => sum + (inv.totalAmount ?? inv.amount ?? 0), 0);
            const paymentRevenue = monthPayments.reduce((sum, pay) => sum + (pay.amount || 0), 0);
            const realMrr = Math.round((invoiceRevenue + paymentRevenue) * 100) / 100;
            monthlyGrowth.push({
                month: monthLabel,
                mrr: realMrr,
    ...
    async exportFinanceCsv() {
        ...
        for (const org of orgs) {
            ...
            const price = org.subscription?.price ?? (PLAN_PRICES[plan] || 99);
    ...
    async getPayments() {
        const payments = await PaymentRecord.find({ isDeleted: { $ne: true } })
            .sort({ paymentDate: -1, createdAt: -1 })
            .limit(100);
        const totalCollected = Math.round(payments.reduce((sum, p) => sum + (p.amount || 0), 0) * 100) / 100;
        return {
            payments: payments.map((p) => ({
                id: p._id.toString(),
                _id: p._id.toString(),
                ...
                recordedAt: p.paymentDate ||
                    p.recordedAt ||
                    p._doc?.recordedAt,
                recordedBy: p.recordedByEmail || p.recordedBy || "Super Admin",
            })),
            totalCollected,
        };
    }
    async recordPayment(body, actorUserId, actorUserEmail) {
        const { invoiceId, invoiceNo, organizationName, organizationId, amount, method, paymentMethod, reference, referenceNumber, notes, } = body || {};
        let invoice = null;
        ...
        if (invoice && paymentAmount > currentBalance) {
            throw new AppError(400, "OVERPAYMENT_NOT_ALLOWED", `Payment amount ($${paymentAmount.toFixed(2)}) exceeds invoice balance due ($${currentBalance.toFixed(2)})`);
        }
        ...
        let normalizedMethod = "bank_transfer";
        if (rawMethod.includes("wire"))
            normalizedMethod = "wire";
        else if (rawMethod.includes("check"))
            normalizedMethod = "check";
        else if (rawMethod.includes("card"))
            normalizedMethod = "manual_card";
        else if (rawMethod.includes("ach") ||
        ...
        let targetOrgId = invoice?.organizationId ||
        ...
        if (invoice) {
            const newPaid = Math.round(((invoice.amountPaid || 0) + paymentAmount + Number.EPSILON) * 100) / 100;
            const totalAmt = invoice.totalAmount ?? invoice.amount ?? 0;
            newBalance = Math.max(0, Math.round((totalAmt - newPaid + Number.EPSILON) * 100) / 100);
    ...
    async getExpenses(query) {
        ...
        const expenses = await ExpenseRecord.find(filter)
            ...
            .limit(100);
        const totalOperatingExpenses = Math.round(expenses.reduce((sum, e) => sum + (e.amount || 0), 0) * 100) / 100;
        return {
            expenses: expenses.map((e) => ({
                ...
                recordedBy: e.createdByEmail || e.recordedBy || "Super Admin",
    ...
    async recordExpense(body, actorUserId, actorUserEmail) {
        const { title, description, amount, currency = "USD", category, vendor, expenseDate, incurredAt, isRecurring = false, receiptUrl, organizationId, notes, } = body || {};
        ...
        if (!validCategories.includes(normalizedCategory)) {
            throw new AppError(400, "INVALID_CATEGORY", `Invalid expense category: '${category}'. Must be one of: ${validCategories.join(", ")}`);
        }
        const expenseAmount = Math.round((Number(amount) + Number.EPSILON) * 100) / 100;
    ...
    async getCustomerAccounts(query) {
        ...
        if (search && typeof search === "string" && search.trim()) {
            ...
            results = results.filter((r) => r.organization.name.toLowerCase().includes(q) ||
        ...
        const summary = {
            ...
            goodStandingCount: results.filter((r) => r.accountStatus === "good_standing").length,
            ...
            totalOutstandingBalance: Math.round(results.reduce((sum, r) => sum + r.currentBalance, 0) * 100) / 100,
        };
        return {
            results,
            accounts: results,
    ...
    async updateCustomerAccount(id, body, actorUserId, actorUserEmail) {
        ...
        const { accountStatus, billingCycle, creditLimit, preferredCurrency, billingContact, commercialNotes, } = body || {};
        if (accountStatus !== undefined) {
            ...
            if (!VALID_STATUSES.includes(accountStatus)) {
                throw new AppError(400, "INVALID_STATUS", `Invalid account status '${accountStatus}'. Must be one of: ${VALID_STATUSES.join(", ")}`);
            }
        ...
        if (billingCycle !== undefined) {
            ...
            if (!VALID_CYCLES.includes(billingCycle)) {
                throw new AppError(400, "INVALID_BILLING_CYCLE", `Invalid billing cycle '${billingCycle}'. Must be one of: ${VALID_CYCLES.join(", ")}`);
            }
        ...
        if (billingContact && typeof billingContact === "object") {
            account.billingContact = {
                ...
                address: billingContact.address !== undefined
    ...
    // ---------------------------------------------------------------------------
    async getOnboardingCases(query) {
        const { organizationId, state, page = "1", limit = "15", } = query || {};
        const pageNum = Math.max(1, parseInt(page) || 1);
        const limitNum = Math.min(100, Math.max(1, parseInt(limit) || 15));
        const skip = (pageNum - 1) * limitNum;
        const filter = { isDeleted: false };
        ...
        return {
            cases: cases.map((c) => ({
                ...
                employee: c.employeeId
                    ? {
                        id: c.employeeId._id?.toString(),
                        name: c.employeeId.profile?.fullName || "Employee",
                        email: c.employeeId.auth?.email,
                        department: c.employeeId.employment?.department || "General",
                    }
                    : null,
                organization: c.organizationId
                    ? {
                        id: c.organizationId._id?.toString(),
                        name: c.organizationId.name,
                        slug: c.organizationId.slug,
                    }
                    : null,
                source: c.source,
                state: c.state,
            ...
            summary: {
                totalCases: Object.values(stateMap).reduce((a, b) => a + Number(b), 0),
                activeCases: Number(stateMap["active"] || 0) +
                    Number(stateMap["ready"] || 0) +
                    Number(stateMap["provisioning"] || 0),
                readyForHandover: Number(stateMap["ready_for_handover"] || 0) +
    ...
    async getTasksOps(query) {
        const { organizationId, status, type, page = "1", limit = "15", } = query || {};
        const pageNum = Math.max(1, parseInt(page) || 1);
        const limitNum = Math.min(100, Math.max(1, parseInt(limit) || 15));
        const skip = (pageNum - 1) * limitNum;
        const filter = { isDeleted: false };
            ...
            tasks: tasks.map((t) => ({
                ...
                assignee: t.assignedToUserId
                    ? {
                        ...
                        name: t.assignedToUserId.profile?.fullName || "User",
                        email: t.assignedToUserId.auth?.email,
                    }
                    : null,
                organization: t.organizationId
                    ? {
                        id: t.organizationId._id?.toString(),
                        name: t.organizationId.name,
                        slug: t.organizationId.slug,
                    }
                    : null,
                createdAt: t.createdAt,
            })),
    ...
    async getActivity(query) {
        const { organizationId, category, severity, search, page = "1", limit = "25", } = query || {};
        const pageNum = Math.max(1, parseInt(page) || 1);
        ...
        return {
            events: logs.map((l) => ({
                ...
                actor: l.actorUserId
                    ? {
                        id: l.actorUserId._id?.toString(),
                        name: l.actorUserId.profile?.fullName || "User",
                        email: l.actorUserId.auth?.email,
                    }
                    : { name: "System Operator", email: "system@talnova.app" },
                organization: l.organizationId
                    ? {
                        id: l.organizationId._id?.toString(),
                        name: l.organizationId.name,
                        slug: l.organizationId.slug,
                    }
    ...
    async getAiObservability() {
        ...
        const dominantModel = dominantModelAgg.length > 0
    ...
    async getAiUsage(query) {
        const { organizationId, feature, provider, model, status, page = 1, limit = 50, } = query || {};
        const filter = {};
        if (organizationId && mongoose.Types.ObjectId.isValid(organizationId)) {
            filter.organizationId = new mongoose.Types.ObjectId(organizationId);
        }
            ...
            try {
                ...
                if (rawDocs && rawDocs.length > 0) {
                    for (const raw of rawDocs) {
                        ...
                        if (rawKey) {
                            await FeatureFlag.updateOne({ key: rawKey }, {
                                $setOnInsert: {
    ...
    async createFlag(body, actorUserId) {
        const { key, name, description, isEnabled, enabled, environment = "all", targetAudience = "global", targetOrganizationIds = [], excludedOrganizationIds = [], targetRoles = [], rolloutPercentage, rolloutPct, } = body || {};
        ...
        const newFlag = await FeatureFlag.create({
            ...
            environment,
            targetAudience,
            targetOrganizationIds: targetOrganizationIds.map((id) => new mongoose.Types.ObjectId(id)),
            excludedOrganizationIds: excludedOrganizationIds.map((id) => new mongoose.Types.ObjectId(id)),
            targetRoles,
    ...
    async updateFlag(key, body, actorUserId) {
        const normalizedKey = (key || "").trim().toLowerCase();
        ...
        if (body.targetOrganizationIds !== undefined) {
            ...
            flag.targetOrganizationIds = body.targetOrganizationIds.map((id) => new mongoose.Types.ObjectId(id));
        }
        if (body.excludedOrganizationIds !== undefined) {
            ...
            flag.excludedOrganizationIds = body.excludedOrganizationIds.map((id) => new mongoose.Types.ObjectId(id));
        }
        if (body.targetRoles !== undefined) {
            if (!Array.isArray(body.targetRoles)) {
                throw new AppError(400, "VALIDATION_ERROR", "targetRoles must be an array");
            }
        ...
        if (body.rolloutPercentage !== undefined || body.rolloutPct !== undefined) {
            ...
            if (isNaN(pct) || pct < 0 || pct > 100) {
                throw new AppError(400, "VALIDATION_ERROR", "rolloutPercentage must be between 0 and 100");
            }
        ...
        if (body.name !== undefined)
            flag.name = String(body.name).trim();
    ...
    async getAlerts(query) {
        const { status, severity, category, search, page = 1, limit = 50, } = query || {};
    ...
    async updateAlertStatus(id, body, actorUserId) {
        const { status, resolutionNotes } = body || {};
        if (!status ||
            !["open", "acknowledged", "investigating", "resolved", "ignored"].includes(status)) {
            throw new AppError(400, "BAD_REQUEST", "Invalid alert status. Must be one of: open, acknowledged, investigating, resolved, ignored");
        }
    ...
    async exportReport(reportId, format, actorUserId) {
        if (!ReportGeneratorService.isSupportedReport(reportId)) {
            throw new AppError(404, "NOT_FOUND", `Report '${reportId}' not found in canonical reports catalog.`);
        }
    ...
    async updatePlatformSettings(body, actorUserId) {
        const { maintenanceMode, maintenanceMessage, sessionTimeoutMinutes, enforceMfaAdmins, } = body || {};
        if (sessionTimeoutMinutes !== undefined) {
            const timeout = Number(sessionTimeoutMinutes);
            if (isNaN(timeout) || timeout < 5 || timeout > 1440) {
                throw new AppError(400, "BAD_REQUEST", "Session timeout must be between 5 and 1440 minutes");
            }
        }
