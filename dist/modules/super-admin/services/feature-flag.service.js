...
export class FeatureFlagService {
     ...
     */
    static async isEnabled(key, organizationId, role) {
        ...
        if (orgStr && flag.excludedOrganizationIds && flag.excludedOrganizationIds.length > 0) {
            const isExcluded = flag.excludedOrganizationIds.some((id) => (id._id || id).toString() === orgStr);
            if (isExcluded) {
                return false;
            }
        }
        ...
        if (orgStr && flag.targetOrganizationIds && flag.targetOrganizationIds.length > 0) {
            const isTargeted = flag.targetOrganizationIds.some((id) => (id._id || id).toString() === orgStr);
            if (isTargeted) {
                return true;
            }
        }
        ...
        if (flag.targetAudience === "roles") {
            if (role && flag.targetRoles && flag.targetRoles.length > 0) {
                return flag.targetRoles.includes(role);
            }
            return false;
        }
     ...
     */
    static async getAllResolvedFlags(organizationId, role) {
        try {
