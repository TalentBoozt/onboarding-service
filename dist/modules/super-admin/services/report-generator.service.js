...
export class ReportGeneratorService {
    ...
    static isSupportedReport(reportId) {
        return this.CANONICAL_IDS.has(reportId);
    }
    static async generateReport(reportId, requestedFormat) {
        if (!this.isSupportedReport(reportId)) {
            const err = new Error(`Unsupported report identifier: ${reportId}`);
            err.statusCode = 404;
            throw err;
        }
        const defaultFormat = reportId === "rep-session-security" || reportId === "rep-api-latency" ? "json" : "csv";
        const format = (requestedFormat?.toLowerCase() === "json" ? "json" : requestedFormat?.toLowerCase() === "csv" ? "csv" : defaultFormat);
        let rows = [];
        let headers = [];
        switch (reportId) {
            case "rep-org-growth": {
            ...
            case "rep-payment-reconciliation": {
                ...
                rows = payments.map((p) => {
                    ...
                    return [
                        ...
                        p.currency || "USD",
                        p.paymentMethod,
                        p.referenceNumber || p.reference || "N/A",
            ...
            case "rep-onboarding-funnel": {
                ...
                rows = cases.map((c) => [
                    c._id.toString(),
                    c.employeeId?.profile?.fullName || "N/A",
                    c.employeeId?.auth?.email || "N/A",
                    c.organizationId?.name || "N/A",
                    c.state,
                    c.stateReason || "N/A",
                    c.source,
            ...
            case "rep-hardware-sla": {
                ...
                rows = tasks.map((t) => {
                    ...
                    return [
                        t._id.toString(),
                        t.title,
                        t.organizationId?.name || "N/A",
                        t.assignedToUserId?.profile?.fullName || "Unassigned",
                        t.priority,
                        t.status,
                        t.dueDate || "None",
            ...
            case "rep-security-audit": {
                ...
                rows = logs.map((l) => [
                    ...
                    l.action,
                    l.resourceType,
                    l.actorUserId?.profile?.fullName || l.actorUserId?.auth?.email || "System",
                    l.organizationId?.name || "Platform",
                    l.severity,
                    l.description,
                ]);
                break;
            ...
            case "rep-session-security": {
                ...
                if (format === "json") {
                    const content = JSON.stringify({
                        ...
                        sessions: sessions.map((s) => ({
                            id: s._id.toString(),
                            user: s.userId?.profile?.fullName || "Unknown",
                            email: s.userId?.auth?.email || "Unknown",
                            organization: s.organizationId?.name || "Platform",
                ...
                rows = sessions.map((s) => [
                    s._id.toString(),
                    s.userId?.profile?.fullName || "Unknown",
                    s.userId?.auth?.email || "Unknown",
                    s.organizationId?.name || "Platform",
                    s.ipAddress || "N/A",
                    s.deviceInfo || "N/A",
