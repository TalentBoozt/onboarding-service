...
export class AICourseBuilderService {
     ...
     */
    async generateJourneyOutline(orgId, userId, prompt, targetRole = "Software Engineer", department = "Engineering", level = "Intermediate", moduleCount = 3) {
     ...
     */
    async regenerateModule(orgId, userId, draftId, moduleId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
     ...
     */
    async publishDraftToJourney(orgId, userId, draftId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const userObjectId = new mongoose.Types.ObjectId(userId.toString());
        const draft = await AICourseDraft.findOne({
            _id: new mongoose.Types.ObjectId(draftId),
     ...
     * Get Draft by ID
     */
    async getDraftById(orgId, draftId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
     ...
     * Delete Draft
     */
    async deleteDraft(orgId, draftId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
