...
export class AIAssistantService {
    ...
    gapService = new KnowledgeGapService();
     ...
     */
    async chat(orgId, userId, role, messageText, conversationId) {
        ...
        if (this.structuredDataService.isStructuredQuery(messageText)) {
            const structuredResult = await this.structuredDataService.resolveStructuredQuery(orgObjectId, userObjectId, messageText);
        ...
        if (retrievalResult.hasRelevantKnowledge && retrievalResult.chunks.length > 0) {
            ...
            const evidenceBlocks = retrievalResult.chunks
                .map((c, idx) => `<company_evidence index="${idx + 1}" title="${c.title}" section="${c.heading || ""}">\n${c.content}\n</company_evidence>`)
                .join("\n\n");
...
4. Always cite official document titles when presenting policy information (e.g. [Source: Title]).`;
            if (!activeClient.isFallbackMock) {
                ...
                try {
                    aiContent = await activeClient.service.chatCompletion(activeClient.config, activeClient.secrets, [
     ...
     */
    async getConversations(orgId, userId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const userObjectId = new mongoose.Types.ObjectId(userId.toString());
     ...
     */
    async getConversationById(orgId, userId, conversationId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const userObjectId = new mongoose.Types.ObjectId(userId.toString());
     ...
     */
    async logFeedback(orgId, userId, conversationId, messageId, rating, comment) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId.toString());
        const userObjectId = new mongoose.Types.ObjectId(userId.toString());
     ...
     */
    async summarizeReflection(reflectionText, metadata) {
        ...
        if (orgId) {
            try {
                ...
                if (activeClient && !activeClient.isFallbackMock) {
                    const modelName = activeClient.config?.model || activeClient.config.provider;
...
Required JSON format:
{
  ...
  "modelName": "${modelName}"
                    ...
                    const responseText = await activeClient.service.chatCompletion(activeClient.config, activeClient.secrets, [
                        { role: "system", content: "You are an expert HR coach. Output only valid JSON with no markdown wrapping." },
                        { role: "user", content: prompt },
                    ], { organizationId: orgId, feature: "milestone_reflection" });
        ...
        const hasBlockers = lower.includes("block") ||
        ...
        const isHighConfidence = (metadata?.rating !== undefined && metadata.rating >= 4) ||
