...
import BuddyProfile from "../models/buddy-profile.model.js";
import BuddyAssignment from "../models/buddy-assignment.model.js";
...
export class BuddyService {
     ...
     */
    async registerBuddyProfile(orgId, userId, data) {
        ...
        if (!profile) {
            ...
            if (data.maxMentees)
                profile.maxMentees = data.maxMentees;
            if (data.skills)
     ...
     */
    async getBuddyProfile(orgId, userId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const userObjectId = new mongoose.Types.ObjectId(userId);
     ...
     */
    async listAvailableBuddies(orgId) {
     ...
     */
    async assignBuddy(orgId, newHireUserId, buddyUserId, assignedByUserId, templateName, matchScore, matchCriteria) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        ...
        if (existing) {
            ...
            await existing.save();
            await BuddyProfile.findOneAndUpdate({ organizationId: orgObjectId, userId: existing.buddyUserId }, { $inc: { currentMenteeCount: -1 } });
        }
        ...
        await BuddyProfile.findOneAndUpdate({ organizationId: orgObjectId, userId: buddyObjectId }, { $inc: { currentMenteeCount: 1 } }, { upsert: true });
        ...
        const managerId = newHire.employment?.managerId || newHire.employment?.managerUserId;
     ...
     */
    async getEmployeeBuddy(orgId, newHireUserId) {
     ...
     */
    async getBuddyMentees(orgId, buddyUserId) {
     ...
     */
    async listOrganizationAssignments(orgId) {
        return BuddyAssignment.find({
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        })
     ...
     */
    async updateChecklistTask(orgId, assignmentId, taskId, completed, actingUserId, actingUserRole) {
        const assignment = await BuddyAssignment.findOne({
        ...
        if (item) {
            ...
            if (completed) {
                ...
                if (allCompleted) {
                    ...
                    const mgrId = menteeUser?.employment?.managerId || menteeUser?.employment?.managerUserId;
                    if (mgrId) {
                        notificationService.createNotification({
                            organizationId: orgId,
     ...
     */
    async addCustomChecklistTask(orgId, assignmentId, taskData, actingUserId, actingUserRole) {
        const assignment = await BuddyAssignment.findOne({
            _id: new mongoose.Types.ObjectId(assignmentId),
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
     ...
     */
    async logBuddyCheckin(orgId, assignmentId, payload, actingUserId, actingUserRole) {
        const assignment = await BuddyAssignment.findOne({
            _id: new mongoose.Types.ObjectId(assignmentId),
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        ...
        try {
            ...
            await gamificationService.awardPoints(orgId, assignment.newHireUserId, "buddy_checkin", 25, "Completed 1-on-1 buddy check-in meeting", `buddy_checkin_${assignment._id}_${checkinIdx}`);
        }
        catch (gErr) {
     ...
     */
    calculateCompatibilityScore(newHire, candidateProfile, candidateUser) {
        ...
        const candidateDept = (candidateProfile.department ||
        ...
        if (hireDept && candidateDept && hireDept === candidateDept) {
            ...
            const matchCluster = clusters.find((c) => c.some((k) => hireDept.includes(k)) && c.some((k) => candidateDept.includes(k)));
            if (matchCluster) {
                departmentScore = 0.5;
            }
        }
        ...
        const candLanguages = (candidateProfile.languages && candidateProfile.languages.length > 0
            ? candidateProfile.languages
            : [candidateUser.preferences?.language || "en"]).map((l) => l.trim().toLowerCase());
        const hasSharedLanguage = candLanguages.some((cl) => cl === hireLang || cl.startsWith(hireLang) || hireLang.startsWith(cl));
        ...
        // Jaccard index
        const candSkills = (candidateProfile.skills || []).map((s) => s.trim().toLowerCase());
        const hireSkills = (newHire.skills || (newHire.employment?.jobTitle ? [newHire.employment.jobTitle] : [])).map((s) => s.trim().toLowerCase());
        let skillsScore = 0.4;
        ...
        const composite = 0.35 * departmentScore +
            0.25 * locationScore +
            0.20 * languageScore +
            0.15 * capacityScore +
            0.05 * skillsScore;
     ...
     */
    async autoAssignBuddyToNewHire(orgId, newHireUserId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const newHireObjectId = new mongoose.Types.ObjectId(newHireUserId);
        ...
        const validCandidates = candidateProfiles.filter((p) => p.userId && !p.userId.isDeleted);
        ...
        const assignment = await this.assignBuddy(orgId, newHireObjectId, chosen.candidate.userId._id, chosen.candidate.userId._id, undefined, chosen.score, chosen.criteria);
        return {
            success: true,
            assignmentId: assignment._id.toString(),
     ...
     */
    async scanBuddyCoachingNudges(orgId) {
        const query = {
            status: "active",
            isDeleted: false,
        };
        ...
        for (const assignment of activeAssignments) {
            if (!assignment.buddyUserId || !assignment.newHireUserId)
                continue;
            const newHire = assignment.newHireUserId;
            const buddy = assignment.buddyUserId;
            ...
            const nudges = assignment.coachingNudges;
            let nudgeToSend = null;
            ...
            if (nudgeToSend) {
                ...
                assignment.coachingNudges[nudgeToSend.field] = new Date();
                await assignment.save();
                nudgesSentCount++;
            }
        }
