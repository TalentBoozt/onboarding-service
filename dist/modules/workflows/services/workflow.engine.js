...
export class WorkflowEngine {
     ...
     */
    async processEvent(organizationId, triggerType, targetUserId, eventPayload = {}) {
        ...
        if (triggerType === "user_created") {
            ...
            for (const journey of audienceJourneys) {
                const matchingUsers = await smartAssignmentService.findMatchingEmployees(organizationId, journey.audience);
                const isMatch = matchingUsers.some((u) => u._id.toString() === targetUserId.toString());
                if (isMatch) {
                    let specificity = 0;
                    if (journey.audience?.departmentNames?.length || journey.audience?.departments?.length)
                        specificity++;
                    if (journey.audience?.jobTitleNames?.length || journey.audience?.jobTitles?.length)
                        specificity++;
                    if (journey.audience?.locations?.length)
                        specificity++;
                    if (journey.audience?.employmentTypes?.length)
                        specificity++;
                    matchedRules.push({
                        _id: journey._id,
        ...
        const journeyAssigningRules = matchedRules.filter((r) => r.actions.some((a) => a.type === "assign_journey"));
        ...
        let executedCount = 0;
        let anyJourneyAssigned = matchedRules.some((r) => r.actions.some((a) => a.type === "assign_journey"));
        if (triggerType === "user_created" && !anyJourneyAssigned) {
            ...
            if (defaultJourney) {
                try {
                    await assignmentService.assignJourney(organizationId, targetUser._id, defaultJourney._id, defaultJourney.createdBy, {
        ...
        for (const rule of matchedRules) {
            ...
            for (let i = 0; i < rule.actions.length; i++) {
                ...
                if (action.type === "delay") {
                    ...
                    await queueService.enqueue("resume_delayed_workflow", {
                ...
                try {
                    ...
                    if (res.status === "failed" && overallStatus !== "failed") {
                        ...
                        if (action.type === "assign_journey") {
                            await this.quarantineOnboardingCase(organizationId, targetUser._id, res.message || "Journey assignment failed");
                        }
                    }
                }
                catch (err) {
                    ...
                    if (action.type === "assign_journey") {
                        await this.quarantineOnboardingCase(organizationId, targetUser._id, errorMsg || "Action step failed");
                    }
                }
            }
            if (!isDelayed) {
     ...
     */
    async resumeDelayedExecution(executionId) {
        ...
        for (let i = startIndex; i < rule.actions.length; i++) {
            ...
            try {
                const res = await this.executeAction(action, execution.organizationId, targetUser, rule.createdBy, {});
                execution.stepResults.push({
                    stepIndex,
                    actionType: action.type,
                    status: res.status,
     ...
     */
    async quarantineOnboardingCase(organizationId, employeeId, reason) {
        try {
            ...
            if (caseRecord && caseRecord.state !== "paused") {
                await onboardingCaseService.transition(caseRecord._id.toString(), organizationId.toString(), "paused", undefined, `rule_conflict: ${reason}`);
            }
        }
        catch (err) {
     ...
     */
    evaluateConditions(conditions, targetUser, eventPayload = {}) {
     ...
     */
    async executeAction(action, organizationId, targetUser, authorUserId, eventPayload) {
        const authorIdStr = authorUserId?._id
            ? authorUserId._id.toString()
            : authorUserId.toString();
        switch (action.type) {
            case "assign_journey": {
                const journeyId = action.params?.journeyId ||
                    action.targetTemplateId ||
                    action.targetTemplate ||
                    action.params?.targetTemplateId;
                if (!journeyId) {
                ...
                try {
                    ...
                    const assignment = await assignmentService.assignJourney(organizationId, targetUser._id, journeyId, authorIdStr, {
            ...
            case "create_task": {
                ...
                try {
                    ...
                    const authorObjectId = mongoose.Types.ObjectId.isValid(authorIdStr)
                        ...
                        : targetUser._id;
                    const assignedUserId = await roleChecklistService.resolveResponsibleUser(orgObjectId, targetUser, action.params.taskAssigneeRole, authorObjectId);
            ...
            case "assign_checklist": {
                const checklistTemplateId = action.params?.checklistTemplateId ||
                ...
                try {
                    const applied = await roleChecklistService.applyTemplateToUser(organizationId, checklistTemplateId, targetUser._id, authorIdStr);
                    return {
                        status: "success",
            ...
            case "send_notification": {
                ...
                try {
                    const recipientRole = action.params.recipientRole || "employee";
                    const recipientIds = [];
                    ...
                    if (recipientRole === "manager" || recipientRole === "both") {
                        const mgrId = targetUser.employment?.managerId || targetUser.employment?.managerUserId;
                        if (mgrId)
                            recipientIds.push(mgrId.toString());
                    }
            ...
            case "assign_document": {
                ...
                try {
                    const docAssignment = await documentService.assignDocument(organizationId, action.params.documentTemplateId, targetUser._id, authorIdStr);
                    return {
                        status: "success",
            ...
            case "trigger_buddy": {
                try {
                    if (action.params.buddyUserId) {
                        const assignment = await buddyService.assignBuddy(organizationId, targetUser._id, action.params.buddyUserId, authorIdStr);
                        return {
                            status: "success",
                        ...
                        const assigned = await buddyService.autoAssignBuddyToNewHire(organizationId, targetUser._id);
                        if (!assigned) {
                            return {
                                status: "failed",
            ...
            case "assign_milestone": {
                ...
                try {
                    ...
                    if (!resolvedTemplateId) {
                        ...
                        const query = { organizationId: new mongoose.Types.ObjectId(organizationId), isDeleted: false };
                        if (targetDay) {
                    ...
                    const assignedMilestone = await milestoneService.assignMilestone(organizationId, resolvedTemplateId, targetUser._id, authorIdStr);
                    return {
                        status: "success",
