...
export class JourneyService {
    ...
    async createJourney(orgId, journeyData, userId) {
        ...
        const newJourney = {
            ...
            publishing: { status: (journeyData.status || "published"), version: 1 },
            analytics: {
                totalAssignments: 0,
                totalCompletions: 0,
                completionRate: 0,
        ...
        return this.journeyRepository.create(newJourney);
    }
    async updateJourney(id, orgId, updateData, userId) {
    ...
    async archiveJourney(id, orgId, userId) {
        await this.getJourney(id, orgId);
        const publishing = {
            status: "archived",
    ...
    async duplicateJourney(id, orgId, newTitle, userId) {
        const journey = await this.getJourney(id, orgId);
