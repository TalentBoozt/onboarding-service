...
export class AdvancedJourneyService {
     ...
     */
    async checkJourneyPrerequisites(orgId, userId, journeyId) {
        ...
        const prereqIds = journey.prerequisites.map((p) => p._id);
        ...
        const completedJourneySet = new Set(completedAssignments
            .map((a) => {
        ...
        for (const prereq of journey.prerequisites) {
     ...
     */
    async processAdaptiveBranching(orgId, userId, journeyId, quizScore) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const userObjectId = new mongoose.Types.ObjectId(userId);
        const journeyObjectId = new mongoose.Types.ObjectId(journeyId);
        const journey = await Journey.findOne({
        ...
        const matchingRule = journey.conditionalBranches.find((b) => quizScore >= b.minScore && quizScore <= b.maxScore && b.unlockJourneyId);
     ...
     */
    async cloneJourney(orgId, journeyId, userId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const journeyObjectId = new mongoose.Types.ObjectId(journeyId);
        ...
        const { _id, createdAt, updatedAt, ...rest } = sourceJourney;
     ...
     */
    async reorderCurriculum(orgId, journeyId, moduleOrders) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const journeyObjectId = new mongoose.Types.ObjectId(journeyId);
        const journey = await Journey.findOne({
            _id: journeyObjectId,
     ...
     */
    async dispatchLearningReminders(orgId) {
        const orgObjectId = new mongoose.Types.ObjectId(orgId);
        const now = new Date();
