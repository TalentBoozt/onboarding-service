...
export class SmartAssignmentService {
     ...
     */
    async findMatchingEmployees(orgId, audienceRules) {
        const query = {
     ...
     */
    async previewSmartAssignment(orgId, journeyId) {
     ...
     */
    async executeSmartAssignment(orgId, journeyId, assignedByUserId, options) {
        ...
        for (const emp of unassignedEmployees) {
            try {
                await assignmentService.assignJourney(orgId, emp._id, journeyId, assignedByUserId, {
     ...
     */
    async autoEnrollNewHire(orgId, userId) {
        ...
        for (const journey of journeys) {
            ...
            if (isMatch) {
                try {
                    await assignmentService.assignJourney(orgId, userId, journey._id, journey.createdBy, {
                        dueDate: new Date(Date.now() + (journey.audience.startDateOffsetDays || 14) * 24 * 60 * 60 * 1000),
                    });
                    autoEnrolledCount++;
                }
                catch (err) {
